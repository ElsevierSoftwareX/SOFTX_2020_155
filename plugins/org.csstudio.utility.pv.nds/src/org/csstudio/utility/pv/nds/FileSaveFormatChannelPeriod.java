...
/**
 * Creates new file per block foreach channel
 */
public class FileSaveFormatChannelPeriod extends FileSaveFormat implements Debug {
  ...
  public FileSaveFormatChannelPeriod (Preferences preferences, Label fileLabel) {
  ...
  public boolean saveTextFile (DataBlock block) {
	...
	if (_debug > 8)
	  System.err.println ("FileSaveFormatChannelPeriod.saveTextFile() saved one channel");
      }
	...
	closeAllPrintWriters (printWriter, fileName);
      ...
      if (_debug > 8)
	System.err.println ("FileSaveFormatChannelPeriod.saveTextFile() --> IOException");
      return false;
    }
    return true;
  }
  ...
  void closeAllDataOutputStreamsError (DataOutputStream [] dataOutputStream, IOException e,
				       String [] fileName, int errIndex) {
    int messagesPrinted = 0;
