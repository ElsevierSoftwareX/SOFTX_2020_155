#!/bin/sh
# /etc/dolphin_ix_remote_enable_bootserver.sh
#  - Enable IX switch ports for all adapters on specified front-end
...
do
     ...
     /diskless/root/etc/dolphin_ix_port_control.sh "--enable" $ipcfg ${PTCFG[ix_idx]}
# let it work
    sleep 5
    ix_idx=$(($ix_idx+1))
done
