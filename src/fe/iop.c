...
return(-1);
  ...
  if(status == -1) 
  {
	...
	{
	  ioMemData->digOut[ii] = 0;
	  CDO32Output[ii]= ioMemData->digOut[ii];
	...
	// usleep(1);
	  ...
	  // Wait for completion of DMA of Adc data
		...
		jj = 0;
		   do {
			jj ++;
		   }while(((*packedData & 0x110000) > 0) && (jj < 10000000));
		if (jj == 10000000) {
			stop_working_threads = 1;
		...
		if(*packedData & 0x800f0000) status = 0;
		else status = 16;
		jj = kk + 1;
		diagWord |= status * jj;
		...
		int num_outs = 32;
		ioMemCntr = (clock16K % 64);
// printf("got to io thread %d\n",ioMemCntr);
		for(ii=0;ii<num_outs;ii++)
		{
			adcData[kk][ii] = (*packedData & mask);
			adcData[kk][ii] -= offset;
			// dWord[kk][ii] = iir_filter((double)adcData[kk][ii],feCoeff2x,2,&dHistory[ii+kk*32][0]);
			dWord[kk][ii] = adcData[kk][ii];
			ioMemData->adcVal[kk][ioMemCntr][ii] = adcData[kk][ii];
                        packedData ++;
			dWord[kk][ii] = adcData[kk][ii];
			// if(clock16K == 0) printf("%d %d\n",ii,dWord[kk][ii]);
		}
...
// printf("kk= %d\n",kk);
	...
	for(kk=0;kk<cdsPciModules->doCount;kk++)
	{
		...
		if(cdsPciModules->doType[kk] == ACS_8DIO)
		{
			rioOutput[ii] = ioMemData->digOut[kk];
			if (rioInput[ii] != rioOutput[ii]) {
			  writeIiroDio(cdsPciModules, ii, rioOutput[ii]);
			  rioInput[ii] = readIiroDio(cdsPciModules, ii) & 0xff;
			  ioMemData->digOut[kk] = rioInput[ii];
			}
		}
		if(cdsPciModules->doType[kk] == ACS_16DIO)
		{
			if (rioInput1[ii] != ioMemData->digOut[ii]) {
		...
		if(cdsPciModules->doType[kk] == CON_32DO)
		{
			CDO32Output[ii] = ioMemData->digOut[kk];
			if (CDO32Input[ii] != CDO32Output[ii]) {
printf("New set CDO %d\n",ii);
			  CDO32Input[ii] = writeCDO32l(cdsPciModules, ii, CDO32Output[ii]);
			  ioMemData->digOut[kk] = CDO32Input[ii];
			}
		}
	}
...
int main(int argc, char **argv)
{
	...
	cdsPciModules = (CDS_HARDWARE *)_ipc_shm;
	...
	for(ii=0;ii<cdsPciModules->doCount;ii++)
        {
		printf("dotype = %d\n",cdsPciModules->doType[ii]);
		printf("doInstance = %d\n",cdsPciModules->doInstance[ii]);
	}
        printf("***************************************************************************\n");
