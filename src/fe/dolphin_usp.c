...
unsigned int            segmentSize    = IPC_TOTAL_ALLOC_SIZE;
...
// ************************************************************************************* 
sci_error_t dolphin_init(CDS_HARDWARE *cdspci)
{
 int ii;
...
for(ii=0;ii<2;ii++)
{
    ...
    cdspci->dolphinRead[ ii ] = (volatile unsigned long)readAddr;
    ...
    cdspci->dolphinWrite[ ii ] = (volatile unsigned long)writeAddr;
