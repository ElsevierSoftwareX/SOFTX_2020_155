...
int main(int argc, char *argv[]) {
    ...
    sprintf(
        msgout,
        "Starting framer_nds_client with warning file %s, displayIP %s, server %s:%d\n",
    ...
    while (DataConnect(serverIP, serverPort, userPort, read_data) != 0) {
        ...
        if (i > 25) {
            sprintf(msgout,
                    "can't connect to the data server after 25 tries\n");
            printmessage(-2);
            exit(-2);
        }
    }
    ...
    case REALTIME:
        ...
        if (graphOption == GRAPHTREND)
            ...
            if (nslow) {
                ...
                sprintf(
                    msgout,
                    "STOP! can't play slow channel for refresh rate > 1.\n");
                printmessage(-1);
            } else
    ...
    if (stopped == 0) {
        ...
        if (newReadMode == REALTIME) {
            if (graphOption == GRAPHTREND)
                ...
                if (nslow) {
                    stopped = 1;
                    sprintf(msgout, "STOP! can't play slow channel for refresh "
                                    "rate > 1.\n");
                    printmessage(-1);
                } else
                    processID = DataWriteRealtimeFast();
...
/*----------------------------------------------------------------------*/
void *read_data() {
    /*double chData[NCF*2];  should be global because of the limit of
     * multithread */
    ...
    while (1) {
        ...
        if ((byterecv = DataRead()) == -2) {
            ...
            firsttime = 1;
            for (j = 0; j < windowNum; j++) {
                DataGetChSlope(chName[chMarked[j]], &slope[chMarked[j]],
                               &offset[chMarked[j]], &chstatus[chMarked[j]]);
                dfprintf(stderr, "ch%d status=%d\n", j, chstatus[j]);
                dfprintf(stderr, "%s %f %f\n", chName[chMarked[j]],
                         slope[chMarked[j]], offset[chMarked[j]]);
            }
	      ...
	      printmessage(0);
            ...
            if (paused == 1) { /* Pause Trend: record data in a file */
                ...
                if (errorpause == 0) {
                    for (j = 0; j < windowNum; j++) {
                        if (DataChanType(chName[chMarked[j]]) <=
                            0) { /*if no data */
                            fprintf(fppause, "%d %f\n", trendCount + 1, 0.0);
                        } else {
                ...
                trendCount += (1 + drop);
                       ...
                       -1) { /* skip data while restoring paused trend */
            ...
            /* The following are the general cases */
                ...
                case GRAPHTIME:
                    ...
                    case GMODEMULTI:
                        ...
                        for (j = 0; j < windowNum; j++) {
                            ...
                            irate = DataGetCh(chName[chMarked[j]], chData_0, 0,
                                              complexDataType[chMarked[j]]);
                            if (irate < 0) {
                            ...
                            if (filterOn &&
                                step >= FILTER_WIDTH) { /* filtering */
                                if (firsttime) {
                                    for (i = 0; i < FILTER_WIDTH; i++) {
                                        fhist[j][i] = chData_0[0];
                                    }
                                }
                                movAvgFilter(chData_0, chData_1, fhist[j],
                                             irate, step);
                            ...
                            /* Get the data and write it to files which can be
                             * read by DaDsp  */
                            acntr = 0;
                            if (chNameChange[chMarked[j]]) {
                                for (i = 0; i < ADC_BUFF_SIZE; i++)
                                    tempData[j][i] =
                                        chData[0]; /* show constant if status
                                                      changed */
                            }
                            if (bindex == 0) {
                                sprintf(acetempname,
                                        "/tmp/%dFRAME/framertemp%d_%d", shellID,
                                        j, tempcycle);
                                fptemp = fopen(acetempname, "w");
                            }
                            if (chType[chMarked[j]] == 1 ||
                                chType[chMarked[j]] ==
                                    2) { /* if fast I&II channel */
                                         /* note: fast I channels display
                                            graphRate[j]=min{2*chSize[j],resolution} data
                                            fast II channels have full data always */
                                dataShiftS =
                                    graphRate[chMarked[j]] / refreshRate;
                                /* corr of delay and scale is taken cared in dc3
                                   delay only applies to X<=1 */
                                if (graphMethod == GMODEMULTI)
                                    tempxscaler =
                                        winXScaler[chMarked[chSelect]];
                                else
                                    tempxscaler = winXScaler[chMarked[j]];
                                if (graphRate[chMarked[j]] <
                                    2048) { /* graphRate[j] < 2048 */
                                    if (tempxscaler >= 1)
                                        fPoints = ADC_BUFF_SIZE -
                                                  graphRate[chMarked[j]] *
                                                      tempxscaler;
                                    ...
                                    for (i = dataShiftS;
                                         i < graphRate[chMarked[j]]; i++)
                                        tempData[j][i - dataShiftS] =
                                            tempData[j][i];
                                    acntr = graphRate[chMarked[j]] - dataShiftS;
                                }
                                if (trigOn && chMarked[j] == chanTrig - 1 &&
                                    chType[chMarked[j]] == 1) {
                                    l = 0;
                                    hskip = (2 * chSize[chMarked[j]]) /
                                            graphRate[chMarked[j]];
                                    for (i = 0; i < 2 * chSize[chMarked[j]] /
                                                        refreshRate;
                                         i += hskip) {
                                ...
                                if (bindex == 0) {
                                    ...
                                    if (graphMethod != GMODEMULTI) {
                                        if (maxSec <= 1 ||
                                            winXScaler[chMarked[j]] < 1) {
                                            c = -1.0;
                                        } else { /* c = -min{maxSec,
                                                    winXScaler[j]} */
                                            if (maxSec <
                                                winXScaler[chMarked[j]])
                                                c = -maxSec;
                                            else
                                    ...
                                    if (graphMethod ==
                                        GMODEMULTI) { /* show no xyType */
                                        if (maxSec <= 1 ||
                                            winXScaler[chMarked[chSelect]] < 1)
                                            c = -1.0;
                                        else
                                            c = -winXScaler[chMarked[chSelect]];
                                        if (uniton[chMarked[j]]) {
                                            for (i = fPoints; i < tPoints;
                                                 i++) {
                ...
                case GRAPHTREND:
                    ...
                    case GMODEMULTI:
                        ...
                        if (trendCount >= 43200 - 1) { /* 12 hours */
                            sprintf(msgout, "Trend has been played more than "
                                            "24 hours. Reset at %s.\n",
                                    timestring);
                            printmessage(0);
                        ...
                        for (j = 0; j < windowNum; j++) {
                            if (graphMethod == GMODESTAND) {
                                ...
                                if (DataChanType(chName[chMarked[j]]) <=
                                    0) { /*if no data */
                                    GracePrintf("g%d.s0 point %2f, %2f", j,
                                                (float)(trendCount + 1), 0.0);
                                } else {
                                    irate = DataTrendGetCh(chName[chMarked[j]],
                                                           trenddata);
                                    switch (graphOption) {
                                    case GRAPHTREND:
                                        switch (xyType[chMarked[j]]) {
                                ...
                                if (DataChanType(chName[chMarked[j]]) <=
                                    0) { /*if no data */
                                    GracePrintf("g0.s%d point %2f, %2f", j,
                                                (float)(trendCount + 1), 0.0);
                                } else {
                                    irate = DataTrendGetCh(chName[chMarked[j]],
                                                           trenddata);
                                    for (i = 0; i < irate; i++) {
                                        result = trenddata[i].mean;
                                        GracePrintf("g0.s%d point %2f, %le", j,
                                                    (float)(trendCount + i),
                                                    result);
                                    }
                                }
                ...
                if (timeCount > timelimit) {
                    if (!stopped) {
                        stopped = 1;
                        DataWriteStop(processID);
                        sprintf(msgout, "Display stopped since it has been "
                                        "running more than 12 hours. Press "
                                        "Play to start again.\n");
                        printmessage(2);
                    }
                    for (i = 0; i < 16; i++)
                ...
                if (trigOn && graphOption == GRAPHTIME) {
                    irate =
                        DataGetCh(chName[chMarked[chanTrig - 1]], chData, 0, 1);
                    if (irate < 0) {
                    ...
                    if (trigHow == TRIGABOVE) {
                        for (i = 0; i < k; i++) {
                            if (chData[i] > trigLevel) {
                                sprintf(msgout, "triggered above Level=%f, by "
                                                "Ch.%d data[%d]=%f\n",
                        ...
                        for (i = 0; i < k; i++) {
                            if (chData[i] < trigLevel) {
                                sprintf(msgout, "triggered below Level=%f, by "
                                                "Ch.%d data[%d]=%f\n",
                                        trigLevel, chanTrig, i, chData[i]);
                                printmessage(2);
                                trigApply = 1;
...
checkOpInputs() {
    ...
    while (1) {
        if ((msgint = msgrcv(msqid, &mb, MSQSIZE, 0, 0)) == -1) {
            perror("ERROR msgrcv");
            sprintf(msgout, "Message Queue failed. Current display abandoned. "
                            "Please start another New Connection.\n");
            printmessage(reset);
            quitdisplay(0);
        }
        ...
        case 3: /* signal name selected */
        {
            ...
            if (j < 0) {
                sprintf(msgout,
                        "signal %s for Ch.%d doesn't not exist. Unchanged.\n",
                        mb.mtext, chChange);
                printmessage(-1);
                break;
            }
        ...
        case 5: /* Y axis min */
            ...
            if (globOn) {
                ...
                winScaler[chSelect0] =
                    (winYMax[chSelect0] - winYMin[chSelect0]) / 2.0;
            }
        ...
        case 9: /* channel highlight */
            ...
            /*
               ...
               sprintf ( msgout, "New Chan Error: channel number must be <=
            %d\n", windowNum );
            */
            break;
        ...
        case 22: /* reset Xmgrace */
            ...
            while (DataReConnect(serverIP, serverPort)) {
                ...
                if (i > 25) {
                    sprintf(
                        msgout,
                        "can't re-connect to the data server after 25 tries\n");
                    printmessage(-2);
                    break;
                }
            }
        ...
        case 24: /* Window number select */
            sscanf(mb.mtext,
                   ...
                   &windowNum, &chMarked[0], &chMarked[1], &chMarked[2],
                   &chMarked[3], &chMarked[4], &chMarked[5], &chMarked[6],
                   &chMarked[7], &chMarked[8], &chMarked[9], &chMarked[10],
                   &chMarked[11], &chMarked[12], &chMarked[13], &chMarked[14],
                   &chMarked[15]);
...
void graphout() {
    ...
    if (firsttime || screenChange) {
        ...
        for (i = 0; i < windowNum; i++) {
            ...
            if (winScaler[chMarked[i]] > 0) {
                ...
                GracePrintf("yaxis tick major %le",
                            (winScaler[chMarked[i]] / ytick_maj));
                GracePrintf("yaxis tick minor %le",
                            (winScaler[chMarked[i]] / ytick_min));
            }
            ...
            if (uniton[chMarked[i]] && strcasecmp(chUnit[chMarked[i]], "none"))
            ...
            GracePrintf("view %f, %f,%f,%f", x0[i] + XSHIFT, y0[i], x0[i] + w,
                        y0[i] + h);
            ...
            case 3: /* exp */
                ...
                break;
            default: /* Linear */
                if (winScaler[chMarked[i]] < 0.001 ||
                    winScaler[chMarked[i]] > 10000000.0)
                    GracePrintf("yaxis ticklabel format exponential");
                break;
            }
        }
        if (sigOn || graphOption != GRAPHTIME) {
            ...
            case 3: /* exp */
                ...
                break;
            default: /* Linear */
                if (winScaler[chSelect] < 0.001 ||
                    winScaler[chSelect] > 10000000.0)
                    GracePrintf("yaxis ticklabel format exponential");
                break;
            }
        }
    ...
    GracePrintf("subtitle \"\\-Ch %d:  %s%s\"", (chMarked[0] + 1),
                chName[chMarked[0]], complexSuffix(chMarked[0]));
    GracePrintf("string off");
    ...
    for (i = 1; i < windowNum; i++) {
        ...
        GracePrintf("subtitle \"\\-Ch %d:  %s%s\"", (chMarked[i] + 1),
                    chName[chMarked[i]], complexSuffix(chMarked[i]));
        GracePrintf("clear stack");
    }
    /** g16 **/
    if (sigOn || graphOption != GRAPHTIME) {
        ...
        if (autoon[chSelect])
            GracePrintf("autoscale yaxes");
        GracePrintf("subtitle \"\\1 Ch %d:  %s%s       %s\"", (chSelect + 1),
                    chName[chSelect], complexSuffix(chSelect), timestring);
        GracePrintf("g16 on");
        GracePrintf("string off");
    } else {
        GracePrintf("g16 off");
        if (windowNum == 1)
            GracePrintf("string def \"\\1 DAQS Data Display %d Channel at %s\"",
                        windowNum, timestring);
        else
            GracePrintf(
                "string def \"\\1 DAQS Data Display %d Channels at %s\"",
                windowNum, timestring);
        GracePrintf("string loctype view");
...
void graphtrend(int from, int to) {
    ...
    if (firsttime || screenChange) {
        ...
        if (sigOn) {
            ...
            if (winScaler[chSelect] > 0) {
                ...
                GracePrintf("world ymax %le", ymaximum);
                GracePrintf("yaxis tick major %le",
                            winScaler[chSelect] / ytick_maj);
                GracePrintf("yaxis tick minor %le",
                            winScaler[chSelect] / ytick_min);
            }
            GracePrintf("yaxis tick color 7");
    ...
    if (sigOn) {
        ...
        if (autoon[chSelect])
            GracePrintf("autoscale yaxes");
        GracePrintf("subtitle \"\\-Ch %d:  %s%s  from %s to %s\"",
                    (chSelect + 1), chName[chSelect], complexSuffix(chSelect),
        ...
        GracePrintf("g16 off");
        GracePrintf(
            "string def \"\\1 Trend Display %d Channels from  %s  to  %s\"",
            windowNum, trendtimestring, timestring);
        GracePrintf("string loctype view");
...
void graphmulti() {
    ...
    if (firsttime || screenChange) {
        ...
        GracePrintf("world ymax %le", ymaximum);
        GracePrintf("yaxis tick major %le",
                    (ymaximum - yminimum) / (2 * ytick_maj));
        GracePrintf("yaxis tick minor %le",
                    (ymaximum - yminimum) / (2 * ytick_min));
        GracePrintf("yaxis tick color 7");
    ...
    for (i = 0; i < windowNum; i++) {
        GracePrintf("s%d color %d", i, gColor[chMarked[i]]);
        if (uniton[i])
            GracePrintf("legend string %d \"%d: %s (%s) \"", i, (i + 1),
                        chName[chMarked[i]], chUnit[chMarked[i]]);
        else
            GracePrintf("legend string %d \"%d: %s \"", i, (i + 1),
                        chName[chMarked[i]]);
    }
    {
        int scaled = 0;
...
void graphtrmulti(int from, int to) {
    ...
    GracePrintf("xaxis tick major %f", (to - from) / 5.0);
    GracePrintf("xaxis tick minor %f", (to - from) / 20.0);
    GracePrintf("subtitle \"\\1 Trend from %s to %s\"", trendtimestring,
                timestring);
    for (i = 0; i < windowNum; i++) {
        GracePrintf("s%d color %d", i, gColor[i]);
        GracePrintf("legend string %d \"Ch %d:%s%s \"", i, (i + 1),
                    chName[chMarked[i]], complexSuffix(chMarked[i]));
    }
    for (i = 0; i < windowNum; i++) {
        if (autoon[chMarked[i]])
