...
char  timestring[128];
char  starttime[128], firsttimestring[128], lasttimestring[128];
...
FILE   *fd;
	...
	if ( isgps == 0 ) { /* UTC */
	  sprintf ( starttime, "%02d-%02d-%02d-%02d-%02d-%02d", fStart.year,fStart.month,fStart.day,fStart.hour,fStart.min,fStart.sec );
	...
	for ( j=0; j<windowNum; j++ ) {
	  ...
	  if (strlen(chName[j]) > 5) {
	    int l = strlen(chName[j]) - 4;
	    if (!strcmp(chName[j] + l, ".img")) {
	      cdt = 2;
	      chName[j][l] = 0;
	    }
	    if (!strcmp(chName[j]  + l, ".mag")) {
	      cdt = 3;
	      chName[j][l] = 0;
	    }
	    if (!strcmp(chName[j] + l, ".phs")) {
	      cdt = 4;
	      chName[j][l] = 0;
	    }
	    if (!strcmp(chName[j] + l -1, ".real")) {
	      chName[j][l-1] = 0;
	      cdt = 1;
	    }
	  }
...
double julian, hour;
  ...
  sscanf( timest, "%02d-%02d-%02d-%02d-%02d-%02d", &yy,&mm,&dd,&hh,&mn,&ss );
  if ( yy == 98 || yy == 99 )
    yy += 1900;
  else
    yy += 2000;
...
int     monthdays[13] = {31,31,28,31,30,31,30,31,31,30,31,30, 31};
  ...
  sprintf ( timestout, "%02d-%02d-%02d-%02d-%02d-%02d", yy,mm,dd,hh,mn,ss  );
  return;
}
...
int totalsec, tmp, tot1, tot2, ii;
  ...
  if (strcmp(timest1, timest2) == 0) 
    return 0;
  sscanf( timest1, "%02d-%02d-%02d-%02d-%02d-%02d", &yy1,&mm1,&dd1,&hh1,&mn1,&ss1 );
  sscanf( timest2, "%02d-%02d-%02d-%02d-%02d-%02d", &yy2,&mm2,&dd2,&hh2,&mn2,&ss2 );
  /* leap year */
  if ((yy1+2000) % 4 == 0) 
    monthdays1[2] = 29;
  else
