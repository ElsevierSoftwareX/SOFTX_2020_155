...
main (int argc, char *argv[])
{
   ...
   {				/* UTC */
	 ...
	 if (fStart.year < 0)
      ...
      (void) snprintf (starttime, sizeof (starttime),
		       ...
		       fStart.sec);
      duration =
	 (time_t) (fStop.sec + fStop.min * 60 +
		   (fStop.hour + fStop.day * 24) * 3600);
      if (debug != 0)
   ...
   {				/* GPS */
	 ...
	 gpstimest -= duration;
      ...
      (void) snprintf (starttime, sizeof (starttime), "%ld",
		       (long) gpstimest);
   }
   ...
   if (DataConnect (serverIP, serverPort, userPort, read_block) != 0)
   {
      if (copies < 0)
	 fprintf (stderr,
		  "LongPlayBack: can't connect to the data server %s:%d. Make sure the server is running.\n",
		  serverIP, serverPort);
      else
	 fprintf (stderr,
		  "LongPlayBack Ch.%d: can't connect to the data server %s:%d. Make sure the server is running.\n",
   ...
   case PLAYTREND:
	 ...
	 fprintf (stderr,
		  "Request second-trend Ch.%d: time %s, duration %d\n",
		  chNum[0], starttime, duration);
#endif
   ...
   case PLAYTREND60:
	 ...
	 fprintf (stderr,
		  "Request minute-trend Ch.%d: time %s, duration %d\n",
		  chNum[0], starttime, duration);
#endif
      fprintf (stderr, "T0=%s; Length=%d (s)\n", starttime, (int) duration);
      break;
...
read_block ()
{
   ...
   for (j = 0; j < windowNum; j++)
   {
	 ...
	 if (dmax != 0)
	 {
	    ...
	    {
	       fprintf (stderr,
			"Unexpected error when opening files. Exit.\n");
	       DataQuit ();
	       if (chData != NULL)
		  free(chData) ;
	       return (void *) NULL;
	 ...
	 if (dmean != 0)
	 {
	    (void) snprintf (tempfile, (size_t) FILENAME_MAX, "%s%dmean", filename,
			     j);
	    if (debug != 0)
   ...
   while (1)
   {
	 ...
	 for (j = 0; j < windowNum; j++)
	 {
	    if (strcmp (chUnit[j], "no_conv.") == 0)
	    {
	       ...
	       (void) DataGetChSlope (chName[j], &slope[j], &offset[j],
				      &chstatus[j]);
	 }
      }
      else if (bytercv < 0)
	 ...
	 if (ngps - ngps0 - totaldata > 0)
	 {
	    ...
	    if (copies < 0)
	       ...
	       fprintf (stderr,
			"           WARNING Ch.%d: data skipped: %d sec.\n",
	 ...
	 else
	 {
	    ...
	    {
	       fprintf (stderr,
			"Failed to allocate memory, file %s line %d\n",
			__FILE__, __LINE__);
	       exit (EXIT_FAILURE);
	    }
	    ...
	    for (j = 0; j < windowNum; j++)
	    {
		  ...
		  default:
			...
			for (i = 0; i < irate; i++)
			{
			   ...
			   fprintf (fdM[j], "%2f\t%le\n", xTick[i],
				    offset[j] + slope[j] * trendsec[i].max);
			}
		     }
		     if (dmean != 0)
		     {
			for (i = 0; i < irate; i++)
			{
			   ...
			   fprintf (fd[j], "%2f\t%le\n", xTick[i],
				    offset[j] + slope[j] * trendsec[i].mean);
			}
		     }
		     if (dmin != 0)
		     {
			for (i = 0; i < irate; i++)
			{
			   ...
			   fprintf (fdm[j], "%2f\t%le\n", xTick[i],
				    offset[j] + slope[j] * trendsec[i].min);
			}
		     }
		     break;
		  }
		  ...
		  if (dmax != 0)
		  {
			...
			fprintf (stderr, "  read_block() - dmean != 0, write %e to file, line %d\n", trendsec[0].max, __LINE__);	/* JCB */
		     fprintf (fdM[j], "%ld %2f %le\n", skipstatus, xTick[0],
			      offset[j] + slope[j] * trendsec[0].max);
		     for (i = 1; i < irate; i++)
		     {
			if (debug != 0)
			   fprintf (stderr, "  read_block() - dmax != 0, write %e to file, line %d\n", trendsec[i].max, __LINE__);	/* JCB */
		  ...
		  if (dmean != 0)
		  {
			...
			fprintf (stderr, "  read_block() - dmean != 0, write %e to file, line %d\n", trendsec[0].mean, __LINE__);	/* JCB */
		     fprintf (fd[j], "%ld %2f %le\n", skipstatus, xTick[0],
			      offset[j] + slope[j] * trendsec[0].mean);
		     for (i = 1; i < irate; i++)
		     {
			if (debug != 0)
			   fprintf (stderr, "  read_block() - dmean != 0, write %e to file, line %d\n", trendsec[i].mean, __LINE__);	/* JCB */
		  ...
		  if (dmin != 0)
		  {
			...
			fprintf (stderr, "  read_block() - dmin != 0, write %e to file, line %d\n", trendsec[0].min, __LINE__);	/* JCB */
		     fprintf (fdm[j], "%ld %2f %le\n", skipstatus, xTick[0],
			      offset[j] + slope[j] * trendsec[0].min);
		     for (i = 1; i < irate; i++)
		     {
			if (debug != 0)
			   fprintf (stderr, "  read_block() - dmin != 0, write %e to file, line %d\n", trendsec[i].min, __LINE__);	/* JCB */
   ...
   if (dataRecv != 0)
   {
	 ...
	 for (j = 0; j < windowNum; j++)
	 {
	    ...
	    if (dmax != 0)
	    {
		  ...
		  fprintf (stderr, "calling close(), line %d\n", __LINE__);	/* JCB */
	       (void) fclose (fdM[j]);
	       (void) snprintf (tempfile, (size_t) FILENAME_MAX, "%s%dmax", filename,
				j);
	       if (dcstep == 1)
		  GracePrintf ("read xy \"%s\"", tempfile);
	       else
	    ...
	    if (dmin != 0)
	    {
		  ...
		  fprintf (stderr, "calling close(), line %d\n", __LINE__);	/* JCB */
	       (void) fclose (fdm[j]);
	       (void) snprintf (tempfile, (size_t) FILENAME_MAX, "%s%dmin", filename,
				j);
	       if (dcstep == 1)
		  GracePrintf ("read xy \"%s\"", tempfile);
	       else
...
#if 0
	    if (copies < 0)
	       fprintf (stderr, "Total data skipped: %d seconds\n", skip);
	    else
	       fprintf (stderr, "Ch.%d total data skipped: %d seconds\n",
			chNum[0], skip);
#endif
	    fprintf (stderr,
		     "%ld seconds worth of data was unavailable on this server\n",
		     skip);
	 }
      }
      if (playMode == PLAYTREND60)
...
#if 0
	    fprintf (stderr,
		     "LongPlayBack: total %9.1f minutes (%d seconds) of data displayed\n",
		     (float) totaldata / hr, totaldata);
	 else
	    fprintf (stderr,
		     "LongPlayBack Ch.%d: total %9.1f minutes (%d seconds) of data displayed\n",
	 ...
	 if (copies < 0)
	    fprintf (stderr,
		     "LongPlayBack: total %d seconds of data displayed\n",
		     totaldata);
	 else
	    fprintf (stderr,
		     "LongPlayBack Ch.%d: total %d seconds of data displayed\n",
		     chNum[0], totaldata);
#endif
   ...
   if (warning != 0)
   {
      if (copies < 0)
	 fprintf (stderr,
		  "Warning: 0 as input of log, put 1.0e-20 instead\n");
      else
	 fprintf (stderr,
		  "Warning Ch.%d: 0 as input of log, put 1.0e-20 instead\n",
		  chNum[0]);
   }
...
graphout (int width, short skip)
{
   ...
   for (i = 0; i < windowNum; i++)
   {
      ...
      GracePrintf ("view %f, %f,%f,%f", x0[i] + XSHIFT, y0[i] + YSHIFT,
		   x0[i] + w, y0[i] + h);
   ...
   for (i = 0; i < windowNum; i++)
   {
      ...
      if (playMode == PLAYDATA)
	 GracePrintf ("subtitle \"Ch %d: %s%s\"", chNum[i], chName[i],
		      complexSuffix (i));
      else
   ...
   if (playMode == PLAYDATA)
      GracePrintf ("string def \"\\1 Data Display start at %s (%d seconds)\"",
		   firsttimestring, width);
   else if (xaxisFormat == XAXISUTC)
      GracePrintf ("string def \"\\1 Trend from %s to %s \"", firsttimestring,
		   lasttimestring);
   else
   {
      if (playMode == PLAYTREND)
	 GracePrintf ("string def \"\\1 Trend %d seconds from %s to %s\"",
		      width, firsttimestring, lasttimestring);
      else if (playMode == PLAYTREND60)
	 GracePrintf ("string def \"\\1 Trend %d minutes from %s to %s\"",
		      width, firsttimestring, lasttimestring);
   }
...
graphmulti (int width, short skip)
{
   ...
   {
      GracePrintf ("s%d color %d", i, gColor[i]);
      GracePrintf ("legend string %d \"Ch %d:%s%s\"", i, chNum[i], chName[i],
		   complexSuffix (i));
      if (xyType[i] == 3)
	 GracePrintf ("yaxis ticklabel format exponential");
   ...
   if (playMode == PLAYDATA)
      GracePrintf
	 ("subtitle \"Display Multiple Data start at %s (%d seconds)\"",
	  firsttimestring, width);
   else
   {
      if (dmin != 0)
	 ...
	 strcpy (tempstr, "Display Multiple MEAN Trend");
      if (xaxisFormat == XAXISUTC)
      {
	 GracePrintf ("subtitle \"%s from %s to %s\"", tempstr,
		      firsttimestring, lasttimestring);
      }
      else
      {				/* XAXISGTS */
	 if (playMode == PLAYTREND)
	    GracePrintf ("subtitle \"%s %d seconds from %s to %s\"", tempstr,
			 width, firsttimestring, lasttimestring);
	 else if (playMode == PLAYTREND60)
	    GracePrintf ("subtitle \"%s %d minutes from %s to %s\"", tempstr,
			 width, firsttimestring, lasttimestring);
      }
   }
   GracePrintf ("legend loctype view");
...
juliantime (char *timest, int extrasec)
{
   ...
   (void) sscanf (timest, "%02d-%02d-%02d-%02d-%02d-%02d", &yy, &mm, &dd, &hh,
		  &mn, &ss);
   if (yy == 98 || yy == 99)
      yy += 1900;
   else
...
timeinstring (char *timest, int extrasec, char *timestout)
{
   ...
   (void) sprintf (timestout, "%02d-%02d-%02d-%02d-%02d-%02d", yy, mm, dd, hh,
		   mn, ss);
   return;
}
