...
sub frontEndCode {
	...
	$longCalculation = <<END;
\t\t\L$::xpartName[$i]\E_temp = (pLocalEpics->$::systemName\.$::xpartName[$i]\_TRAMP + (double) cycle_gps_time + (((double) cycleNum) / ((double) FE_RATE)) - $::fromExp[0]) * \L$::xpartName[$i]_freq_request;
...
END
	...
	$calcExp .= "\t\t\L$::xpartName[$i]_freq_cycle_count = 1;\n";
	$calcExp .= "\t\t\L$::xpartName[$i]_freq_max_count = 0;\n";
	$calcExp .= "\t\t\L$::xpartName[$i]\E_temp = ((double) cycle_gps_time + (((double) cycleNum) / ((double) FE_RATE)) - $::fromExp[0]) * \L$::xpartName[$i]_freq_request;\n";
