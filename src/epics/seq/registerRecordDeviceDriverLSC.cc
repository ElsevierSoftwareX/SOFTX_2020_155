...
epicsShareExtern void (*pvar_func_lscRegistrar)(void);

int lsc_registerRecordDeviceDriver(DBBASE *pbase)
{
    registerRecordTypes(pbase, 7, recordTypeNames, rtl);
    registerDevices(pbase, 7, deviceSupportNames, devsl);
    (*pvar_func_lscRegistrar)();
    return 0;
}
...
static const iocshFuncDef registerRecordDeviceDriverFuncDef =
                {"lsc_registerRecordDeviceDriver",1,registerRecordDeviceDriverArgs};
