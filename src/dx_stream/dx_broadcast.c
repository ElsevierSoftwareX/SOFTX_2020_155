...
Usage( )
{
    ...
    printf( "reflective  -client -nodes <nodes>[ -a <adapter no> -size "
            "<segment size> ] \n" );
    printf( "reflective  -server -rank <rank> [ -a <adapter no> -size <segment "
            "size> ] \n\n" );
    printf( " -client            : %s\n",
    ...
    printf(
        " -rank <value>      : Rank of server nodes (1,2,3,4,5,6,7, 8,9)\n" );
...
sci_error_t
create_and_test_reflective_memory( int client )
{
     ...
     *
     * All nodes within the broadcast group must have the same segmentId to
     * communicate.
     */
    ...
    /* Prepare the segment */
    SCIPrepareSegment(
        localSegment, localAdapterNo, SCI_FLAG_BROADCAST, &error );
    ...
    writeAddr = SCIMapRemoteSegment( remoteSegment,
                                     &remoteMap,
                                     offset,
                                     segmentSize,
                                     NULL,
    ...
    do
    {
        ...
        if ( client )
        {
            ...
            if ( verbose )
            {
                printf( "Send broadcast message (value %d) to %d available "
                        "nodes ...\n",
                        written_value,
                        nodes );
            }
            ...
            dataBuff =
                (unsigned char*)shmDataPtr[ 0 ] + ( lastCycle * buf_size );
            // dataBuff = (unsigned char *)shmDataPtr[0]; // + (lastCycle *
            // buf_size);
            drIntData = (int*)drDataPtr;
            drIntData += 2;
            ...
            *writeAddr = written_value;
            ...
            for ( node_offset = 1; node_offset <= nodes; node_offset++ )
            {
                ...
                do
                {
                    ...
                    {
                        printf( "Value = %d (expected = %d) delayed from rank "
                                "%d - after %u reads\n",
                                value,
                                written_value + 1,
                                node_offset,
            ...
            {
                printf(
                    "Client received broadcast value %d from all nodes \n\n",
                    value );
            }
            written_value++;
        }
        else
        {
            ...
            written_value++;
            /* Lets write a value back received value +1 to the client, offset
             * "myrank" */
    ...
    main( int argc, char* argv[] )
{
    ...
    if ( client )
    {
        ...
        // shmIpcPtr[0] = (struct rmIpcStr *)((char *)dcu_addr +
