//
///	@file mx_crc_only.c
///	@brief  Dummy code to run RCG V3.1 and later in 'standiop' mode ie standalone system
///	< This is stripped down version of mx_stream. Purpose is to provide crc checksum for
///	< DAQ on a standalone system.
//
...
do {
	...
	do {
		...
		for (unsigned int i = 0; i < nsys; i++) {
		  ...
		  printf("Xmit time = %u\t%u\n",shmIpcPtr[i]->bp[lastCycle].timeSec,shmIpcPtr[i]->bp[lastCycle].timeNSec);
		  printf("DCU ID = %d\tCycle = %d\n",shmIpcPtr[i]->dcuId,shmIpcPtr[i]->cycle);
		  ...
		  /// Calculate CRC checksum on data : NOTE shmIpcPtr[i]->bp[lastCycle].crc contains crc length info
