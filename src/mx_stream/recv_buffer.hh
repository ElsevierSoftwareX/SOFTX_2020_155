...
struct gps_key {
    ...
    gps_key(key_type sec, cycle_type cycle): key((static_cast<key_type>(sec) << shift_amount()) | (cycle & cycle_mask()) ) {}
    ...
    cycle_type cycle() const
    {
        return static_cast<cycle_type>(key & cycle_mask());
    }

    bool operator==(const gps_key& other) const
    {
    ...
    {
        key = __sync_or_and_fetch(const_cast<key_type*>(&(source->key)), static_cast<key_type>(0));
    }
    ...
    atomic_store_to(gps_key* dest)
    {
        ...
        while (old_key != *this)
        {
            old_key.key = __sync_val_compare_and_swap(&(dest->key), old_key.key, key);
        }
    }
...
private:
    ...
    static constexpr key_type shift_amount()
    {
        return 4;
    }

    static constexpr cycle_type cycle_mask()
    {
...
struct buffer_entry
{
    buffer_entry(): m(), latest(), ifo_data(), data(&ifo_data.dataBlock[0]) {}
    std::mutex m;
    gps_key latest;
    ...
    void
    injest( const daq_dc_data_t& input )
    {
        gps_key key(input.header.dcuheader[0].timeSec, input.header.dcuheader[0].cycle);
        ...
        {
            const daq_msg_header_t& cur_header =
                input.header.dcuheader[ i ];
            ...
            if ( data + block_size >
                 array_end( ifo_data.dataBlock ) )
            {
                return;
            }
            std::copy(
                input_data, input_data + block_size, data );
            data += block_size;
...
struct receive_buffer
{
    ...
    void ingest(const daq_dc_data_t& input)
    {
        ...
        buffer_entry& target_buffer = buffer[index];

        gps_key key(input.header.dcuheader[0].timeSec, input.header.dcuheader[0].cycle);

        target_buffer.injest( input );

        key.atomic_store_to_if<std::less<gps_key> >(&latest);
    }

private:


    static int cycle_to_index(gps_key::cycle_type cycle)
    {
        return cycle % N;
    }

    gps_key latest;
    std::array<buffer_entry, N> buffer;
