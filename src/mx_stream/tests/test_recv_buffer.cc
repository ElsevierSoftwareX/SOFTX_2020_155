...
std::shared_ptr<daq_dc_data_t>
create_test_blob(std::vector<unsigned int> dcus, unsigned int gps, int cycle)
{
    std::shared_ptr<daq_dc_data_t> daq_data_p = std::make_shared<daq_dc_data_t>();
    daq_dc_data_t& daq_data(*daq_data_p);
    ...
    for (const auto& dcuid:dcus)
    {
        ...
        header.timeSec = gps;
        header.status = 0;
        header.timeNSec = cycle*static_cast<unsigned int>(1000000000/16);
        header.fileCrc = 42;
        header.dataCrc = 42;
        header.tpCount = 0;
...
{
    std::shared_ptr<receive_buffer<4>> buf = std::make_shared<receive_buffer<4>>();
    std::cout << "receive_buffer<4> has a size of " << sizeof(receive_buffer<4>) << std::endl;
}
...
{
    buffer_entry entry;
    auto daq_p = create_test_blob(std::vector<unsigned int>{5,6,7,8}, 1000000000, 5);
    entry.injest(*daq_p);
...
TEST_CASE("You cannot insert a test structure into a buffer entry if it is too old")
{
    buffer_entry entry;
    auto daq_p = create_test_blob(std::vector<unsigned int>{5,6,7,8}, 1000000000, 5);
    entry.injest(*daq_p);
    ...
    daq_p = create_test_blob(std::vector<unsigned int>{9, 10, 11}, 1000000000, 4);
    entry.injest(*daq_p);
...
TEST_CASE("Buffers clear old entries when a new timestamp is put in")
{
    ...
    daq_p = create_test_blob(std::vector<unsigned int>{9, 10, 11}, 1000000000, 6);
    entry.injest(*daq_p);
...
TEST_CASE("Buffers append in data when it is the same time")
{
    ...
    daq_p = create_test_blob(std::vector<unsigned int>{9, 10, 11}, 1000000000, 5);
    entry.injest(*daq_p);
