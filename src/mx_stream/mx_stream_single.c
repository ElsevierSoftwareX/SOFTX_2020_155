...
usage( )
{
    ...
    fprintf( stderr,
             "sys_name - name of a control system model, e.g. isiham6\n" );
...
receiver( mx_endpoint_t ep, uint32_t match_val, uint32_t filter )
{
    ...
    for ( cur_req = 0; cur_req < NUM_RREQ; cur_req++ )
    {
        ...
        mx_irecv(
            ep, &seg, 1, match_val, MX_MATCH_MASK_NONE, 0, &req[ cur_req ] );
    }
    ...
    do
    {
        for ( count = 0; count < NUM_RREQ; count++ )
        {
            ...
            if ( stat.code != MX_STATUS_SUCCESS )
            {
                ...
                // mx_strstatus(stat.code)); fprintf(stderr, "irecv failed\n");
                myErrorStat = 2;
                // exit(1);
            }
            ...
            if ( !myErrorStat )
            {
                ...
                printf( "data from dcuid %d %d %d %d %d\n",
                        dataPtr->mxIpcData.dcuId,
                        dataPtr->mxIpcData.bp[ ii ].timeSec,
                        dataPtr->mxIpcData.bp[ ii ].timeNSec,
                        ii,
                        jj );
			...
			printf("\n\n  ");
                ...
                // if(myCrc != dataPtr->mxIpcData.bp[ii].crc) printf("CRC error
                // in receiver\n");
                //  printf("crc = 0x%x\n  ",shmIpcPtr->bp[ii].crc);
        ...
        if ( !myErrorSignal )
        {
            do
            {
                ...
                mxDataBlock.mxIpcData.bp[ lastCycle ].timeSec =
                    shmIpcPtr->bp[ lastCycle ].timeSec;
                mxDataBlock.mxIpcData.bp[ lastCycle ].timeNSec =
                    shmIpcPtr->bp[ lastCycle ].timeNSec;
                mxDataBlock.mxIpcData.bp[ lastCycle ].cycle =
                    shmIpcPtr->bp[ lastCycle ].cycle;
                mxDataBlock.mxIpcData.bp[ lastCycle ].crc =
                    shmIpcPtr->bp[ lastCycle ].crc;
                ...
                // if(myCrc != mxDataBlock.mxIpcData.bp[lastCycle].crc)
                // printf("CRC error in sender\n");
                sendLength = header_size + mxDataBlock.mxIpcData.dataBlockSize;
                // printf("send length = %d  total length =
...
main( int argc, char** argv )
{
    ...
    while ( ( c = getopt( argc, argv, "hd:e:f:n:b:r:s:l:N:Vvwx" ) ) != EOF )
        switch ( c )
        {
        case 'd':
            rem_host = optarg;
    ...
    if ( rem_host == NULL )
    {
        ...
        shmDataPtr = (char*)( dcu_addr + CDS_DAQ_NET_DATA_OFFSET );
        shmTpTable =
            (struct cdsDaqNetGdsTpNum*)( dcu_addr +
