...
#define CMSG_LEN(size) (sizeof(struct msghdr) + (size))
  ...
  if (mSpec.parse(mSpecFileName)) {
    ...
    if (mSpec.getDataType() == Spec::RawMinuteTrendData) {
      ...
      if (mSpec.getAddedArchives().size() != 0) { // if there are some added archives
	// If all signals are in a single added archive no need to do combination processing
	if (mSpec.getAddedArchives().size() == 1
	    &&(mSpec.getAddedFlags().end() == 
	       find(mSpec.getAddedFlags().begin(), mSpec.getAddedFlags().end(), std::string("0")))) {
