...
daqd_net::daqd_net (int fd, Spec &spec)
  : ndata (0), buf (0), num_signals (spec.getSignalNames().size()),
    first_time(true), mDataFd(fd), mSpec(spec), transmission_block_size(0),
    subjobReadStateMap(), seq_num(0), mSignalBlockOffsets()
 {
#if 0
  buf = malloc (buf_size);
  if (! buf) {
  ...
  for (int i = 0; i < num_signals; i++)  transmission_block_size += bps[i] * rates[i];
}
...
daqd_net::daqd_net (int fd, Spec &spec, CPT &v)
    ...
    subjobReadStateMap(), seq_num(0), mSignalBlockOffsets()
 {
  ...
  for (CPMI p = v.begin (); p != v.end (); p++) {
    subjobReadStateMap.insert (std::pair<int, SubjobReadState>(p -> first, SubjobReadState(p -> second)));
  }
...
daqd_net::send_reconfig_block()
{
  ...
  for (int k = 0; k < num_signals; k++) {
    written = basic_io::writen(mDataFd, (char *) &reconfig_data[k].signal_offset, sizeof(REAL_4));
    if (written != sizeof(REAL_4)) {
      system_log(1, "failed to send reconfig data; errno=%d (FATAL)", errno);
      return false;
    }
    written = basic_io::writen(mDataFd, (char *) &reconfig_data[k].signal_slope, sizeof(REAL_4));
    if (written != sizeof(REAL_4)) {
      system_log(1, "failed to send reconfig data; errno=%d (FATAL)", errno);
      return false;
    }
...
{
  for (;;) {
    int nfinished  = 0; // The number of finished subjobs with and empty block lists
    ...
    if (nfinished == subjobReadStateMap.size()) return true;

    // If there is at least one reconfig block then need to send output reconfig block
    if (reconfigBlockSeen) {
      // Pop all configuration blocks and record the values
      for (CRMINC p = subjobReadStateMap.begin (); p != subjobReadStateMap.end (); p++) {
	if (p -> second.finished && p -> second.block_list.empty()) continue; // Skip finished jobs
	if (is_reconfig_block (p -> second.block_list.front().data)) {
	  SubjobReadState::BT block = p -> second.block_list.front();
	  unsigned int block_len = block.length - 4 * sizeof(unsigned int);
	  ...
	  if (p -> second.signalIndices.size() * sizeof(reconfig_data_t) != block_len) {
	    ...
	    for (int i = 0; i < p -> second.signalIndices.size(); i++) {
	      memcpy(reconfig_data + p -> second.signalIndices[i], data + i*sizeof(reconfig_data_t), sizeof(reconfig_data_t));
	    }
	  }
	  free (block.data);
      ...
      // Find minimum start and end times for the blocks in the front of the receive queues
      for (CRMINC p = subjobReadStateMap.begin (); p != subjobReadStateMap.end (); p++) {
	if (p -> second.finished && p -> second.block_list.empty()) continue; // Skip finished jobs
	SubjobReadState::BT block = p -> second.block_list.front();
	...
	if (gps + block.dt < start_t) start_t = gps + block.dt;
	if (gps + dt < end_t) end_t = gps + dt;
      }
      DEBUG1(cerr << "start_t=" << start_t << "; end_t=" << end_t << endl);
	...
	return false;
      ...
      DEBUG1(cerr << "imageptr=" << hex << (unsigned long) image << "; image_size=" << dec << image_size << endl);
      ...
      // For each block: write the data to the image, write zeros or split blocks
      for (CRMINC p = subjobReadStateMap.begin (); p != subjobReadStateMap.end (); p++) {
	if (p -> second.finished && p -> second.block_list.empty()) continue; // Skip finished jobs
	SubjobReadState::BT *block = &p -> second.block_list.front();
	...
	if (gps + o_dt < end_t) {  // This block has some data we need
	  // Minus data already sent 
	  dt -= o_dt;

	  if (i_end_t > end_t) { // Partial block
	    // Subtract data length not fitting into the block
	    dt = dt -  (i_end_t - end_t); // This many seconds fits into the current block

	    // Add length of data we are going to send now
	    block->dt += dt;
	  }
	  dt /= 60; // 60 seconds per data point
	  unsigned long inb_offs = 0; // offset into the input block's to the start of this signal's memory
	  for (int i = 0; i < p -> second.signalIndices.size(); i++) {
	    unsigned long signal_offset = mSignalBlockOffsets[p -> second.signalIndices[i]]; // Signal offset in the output block
	    unsigned long signal_bps = mSpec.getSignalBps()[p -> second.signalIndices[i]];
	    unsigned long davail = dt*signal_bps; 	           // This much data is available in the input block
	    unsigned long ins_offs = o_dt/60*signal_bps;             // offset in the input block's signal memory
...
daqd_net::comb_read(int fildes)
{
  ...
  if (iter == subjobReadStateMap.end()) {
    system_log(1, "daqd_net::comb_read() unknown fildes passed: %d", fildes);
    return false;
  }
...
{
  DEBUG1(cerr << "combination processing finished with desc " << fildes << endl);
  CRMINC iter = subjobReadStateMap.find(fildes);
  if (iter == subjobReadStateMap.end()) {
    system_log(1, "daqd_net::comb_read() unknown fildes passed: %d", fildes);
    return false;
...
#endif
{
  ...
  if (dt > 1) {
    if (gps.getSec() < start_time) { 	// first transmission block is incomplete
      start_diff = start_time - gps.getSec();
    }
    if (gps.getSec() + dt > end_time) { // last data transmission block could be truncated, if end_time says it must be
  ...
  DEBUG1(cerr << "start_diff=" << start_diff << " seconds_to_send=" << seconds_to_send << endl);

  if (mSpec.getDataType() == Spec::MinuteTrendData) {
    // devide all times by sixty for the minute trend data (at 1/60 Hz)
    seconds_to_send += start_diff % 60; // Add the remainder of the difference to the total time
    start_diff /= 60;
    if (seconds_to_send % 60) seconds_to_send += 60 - seconds_to_send % 60;
    seconds_to_send /= 60;
    DEBUG1(cerr << "Adjusted: start_diff=" << start_diff << " seconds_to_send=" << seconds_to_send << endl);
  }

  unsigned int header [5]; // data transmission block header
  header [0] = htonl (4 * sizeof (unsigned int) + transmission_block_size*seconds_to_send); // len
  if (mSpec.getDataType() == Spec::MinuteTrendData)  {
    header [1] = htonl (60*seconds_to_send); // block time interval
    header [2] = htonl (gps.getSec() + 60*start_diff); // gps time has to be eq. to `j'
  } else {
    header [1] = htonl (seconds_to_send); // block time interval
    header [2] = htonl (gps.getSec() + start_diff); // gps time has to be eq. to `j'
  }
  header [3] = htonl (gps.getNSec()); // this must be zero the way our DAQ system works
  header [4] = htonl (*seq_num);
  (*seq_num)++;
  ...
  for (int k = 0; k < num_signals; k++) {
	...
	if (average) {
	  DEBUG(1, cerr << "averaging" << samples_per_point << endl);
	  if (bps[k] == 2) {
	    for (int l = 0; l < rates[k]*seconds_to_send; l++)
	      ((short *)sptr) [l] = averaging ((short *)sptr + l * samples_per_point, samples_per_point);
	  } else if (bps[k] == 4) {
	    for (int l = 0; l < rates[k]*seconds_to_send; l++)
	      ((float *)sptr) [l] = averaging ((float *)sptr + l * samples_per_point, samples_per_point);
	  } else if (bps[k] == 8) {
	    for (int l = 0; l < rates[k]*seconds_to_send; l++)
	      ((double *)sptr) [l] = averaging ((double *)sptr + l * samples_per_point, samples_per_point);
	  } else {
	    abort();
	  }
	} else {
    ...
    if (bps[k] == 2) {
      for (int j= 0; j < s/2; j++)
         ((unsigned short *)sptr) [j] = htons(((unsigned short *)sptr) [j]);
    } else if (bps[k] == 4) {
      for (int j= 0; j < s/4; j++)
         ((unsigned int *)sptr) [j] = htonl(((unsigned int *)sptr) [j]);
    } else if (bps[k] == 8) {
      for (int j= 0; j < s/8; j++)
