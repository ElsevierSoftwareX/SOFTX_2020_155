...
template <class charT, class traits = char_traits <charT> >
   ...
   public:
      ...
      /** Constructor.
        ...
        */
      basic_mmbuf (ios_base::openmode which = 
                  ios_base::in | ios_base::out);
      /** Constructor.
          Create a streambuf from a file name. The file must be
          mappable.
        */
      basic_mmbuf (const char* filename, ios_base::openmode which = 
                  ios_base::in | ios_base::out);
      /** Constructor.
          Create a streambuf from a memory region.
        */
      basic_mmbuf (char_type* data, streamsize len, ios_base::openmode which =
                  ios_base::in | ios_base::out);
      /** Destructor.
      ...
      char_type* get_address() {
         return (fMode & ios_base::in) ? this->eback() : this->pbase(); }
      /// Return the memory address
      const char_type* get_address() const {
         return (fMode & ios_base::in) ? this->eback() : this->pbase(); }
      /// Return the data length
      streamsize get_length() const {
         return (fMode & ios_base::in) ? 
...
template <class charT, class traits = char_traits <charT> >
   ...
   public:
      ...
      : istream_type (&fBuf), fBuf (filename, ios_base::in) {
         init (&fBuf);
         if (!fBuf.is_open()) this->setstate (ios_base::failbit); }
      /** Constructor.
          Create a streambuf from a memory region.
        */
      ...
      void open (const char* filename) {
         this->clear(); fBuf.set_map (filename);
         if (!fBuf.is_open()) this->setstate (ios_base::failbit); }
      /// Open
      void open (char_type* data, streamsize len) {
         this->clear(); fBuf.set_region (data, len);
         if (!fBuf.is_open()) this->setstate (ios_base::failbit); }
      /// Close
      void close() {
         if (!fBuf.close()) this->setstate (ios_base::failbit);; }
      /// Block read
      mmstream_type& read (char_type* s, streamsize n) {
...
template <class charT, class traits = char_traits <charT> >
   ...
   public:
      ...
      void close() {
         if (!fBuf.close()) this->setstate (ios_base::failbit);; }
      /// Block write
      mmstream_type& write (const char_type* s, streamsize n) {
...
template <class charT, class traits = char_traits <charT> >
   ...
   public:
      ...
      : stream_type (&fBuf), fBuf (filename, ios_base::in | ios_base::out) {
         init (&fBuf);
         if (!fBuf.is_open()) this->setstate (ios_base::failbit); }
      /** Constructor.
          Create a streambuf from a memory region.
      ...
      : stream_type (&fBuf), fBuf (data, len, ios_base::in | ios_base::out) {
         init (&fBuf);
         if (!fBuf.is_open()) this->setstate (ios_base::failbit); }
...
//______________________________________________________________________________
   #ifndef __GNU_STDC_OLD
template <class charT, class traits>
   basic_mmbuf<charT,traits>::basic_mmbuf (char_type* data, streamsize len, 
                     ios_base::openmode which) 
   #else
   inline basic_mmbuf::basic_mmbuf (char_type* data, streamsize len, 
                     ios_base::openmode which)
   #endif
   : fMode (which)
   {
...
//______________________________________________________________________________
   #ifndef __GNU_STDC_OLD
template <class charT, class traits>
   bool basic_mmbuf<charT,traits>::set_map (const char* filename) 
   #else
   inline bool basic_mmbuf::set_map (const char* filename)
   #endif
   {
      ...
      if (fMap.is_mapped()) {
         ...
         if (fMode & ios_base::in)  this->setg (data, data, data + len);
         if (fMode & ios_base::out) this->setp (data, data + len);
         return true;
      }
      else {
         return false;
...
//______________________________________________________________________________
   #ifndef __GNU_STDC_OLD
template <class charT, class traits>
   bool basic_mmbuf<charT,traits>::set_region (char_type* data, 
                     streamsize len)
   #else
   inline bool basic_mmbuf::set_region (char_type* data, streamsize len)
   #endif
   {
...
//______________________________________________________________________________
   #ifndef __GNU_STDC_OLD
template <class charT, class traits>
   bool basic_mmbuf<charT,traits>::close() 
   #else
   inline bool basic_mmbuf::close()
   #endif
   {
...
template <class charT, class traits>
   ...
   basic_mmbuf::seekoff (off_type off, ios_base::seekdir way,
                     ios_base::openmode which)
   #endif
   {
      ...
      if (which & ios_base::in) {
         if (way == ios_base::beg) newoff = 0;
         if (way == ios_base::cur) newoff = this->gptr() - this->eback();
         if (way == ios_base::end) newoff = this->egptr() - this->eback();
      }
      else if (which & ios_base::out) {
         if (way == ios_base::beg) newoff = 0;
         if (way == ios_base::cur) newoff = this->pptr() - this->pbase();
         if (way == ios_base::end) newoff = this->epptr() - this->pbase();
      }
      else {
