...
Spec::parse(std::string fname)
{
  ...
  for (int line = 1; !in.eof(); line++) {
    ...
    if (idx == s.npos) {
	  ...
	  || cb_idx == s.npos
	  || cb_idx <= ob_idx + 1) {
	//	system_log(1, "%s:%d semantic error; invalid section header", fname.c_str(), line);
	//	return false;
	continue;
      }
      archive_dir = s.substr(ob_idx+1, cb_idx - ob_idx - 1);
      DEBUG(5, std::cerr << "Added archives section: " << archive_dir << std::endl);
      continue;
    }
    key = s.substr(0, idx);
    ...
    if (key == "added_flags") {
      ...
      if (v.size()%2) {
	system_log(1, "%s:%d semantic error; must be even number of values", fname.c_str(), line);
	return false;
      }

      std::vector<std::pair<unsigned long, unsigned long> >archive_gps;
	...
	archive_gps.push_back(std::pair<unsigned long, unsigned long>(gps1, gps2));
      }

      // This is the last line in the archive section -- put the archive in
      AddedArchive a(archive_dir, archive_prefix, archive_suffix, archive_gps);
      if (!a.dir.length() || !a.prefix.length() || !a.suffix.length()) {
	DEBUG(5, std::cerr << archive_dir << " " << archive_prefix << " " << archive_suffix << " " << archive_gps.size() << std::endl);
	system_log(1, "%s:%d: semantic error (incomplete added archives section)", fname.c_str(), line);
	return false;
      }
	...
	if (*endptr) {
	  system_log(1, "%s:%d: syntax error (not a number)", fname.c_str(), line);
	  return false;
	}
	...
	if (!power_of(lval,2)) {
	  system_log(1, "%s:%d: syntax error (not power of 2)", fname.c_str(), line);
	  return false;
	}
	...
	mArchiveGps.push_back(std::pair<unsigned long, unsigned long>(gps1, gps2));
      }
    } else {
	system_log(5, "unknown key `%s' in %s:%d", key.c_str(), fname.c_str(), line);
    }
  }

  if (mDataType == TrendData) { // sampled at 1Hz always
    mSignalRates.clear();
  ...
  if ((mSignalNames.size() != mSignalRates.size()
    ...
    {
      system_log(1,"%s: semantic error;  vector length mismatch", fname.c_str());
      return false;
    }
  ...
  if (mDataType == RawMinuteTrendData
    ...
    {
      system_log(1,"%s: semantic error; conversion data vector length mismatch", fname.c_str());
      return false;
    }


  if ((mDataType == MinuteTrendData
       || mDataType == RawMinuteTrendData)
      && mSignalNames.size() != mSignalRates.size()) {
    // All signals will be sampled at 1 (actual sampling rates is 1/60)
    setSignalRates(std::vector<unsigned int>(mSignalNames.size(), 1));
  }
  ...
  if (mDataType == TrendData || mDataType == FullData) {
    ...
    for (int i = 0 ; i < num_channels; i++) {
      int idx = mSignalNames[i].find(":DMT", 0); // See if this is DMT channel
      if (idx != std::string::npos) { // Found a DMT channel
	  // Bad request
	  system_log(1,"%s: DMT request denied, not minute trend\n", fname.c_str());
	  return false;
      }
    }
  }
