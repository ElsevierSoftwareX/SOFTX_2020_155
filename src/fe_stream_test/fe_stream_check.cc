 ...
 * @param[out] meta_data Pointer to option meta data info, to record dcu id and such...
 * @param[out] output destination for channel lists
 */
void
read_channels_from_file(const std::string& fname, model_meta_data* meta_data,
    std::vector<GeneratorPtr>& output)
{
    ...
    while (std::getline(is, line))
    {
        ...
        if (line.front() != '[' || line.back() != ']')
        {
            ...
            if (line.size() > 6 and strncmp("dcuid=", line.c_str(), 6) == 0)
            {
                meta_data->dcuid = extract_int_param(line);
            }
            if (line.size() > 9 and strncmp("datarate=", line.c_str(), 9) == 0)
            {
...
{
    std::cout << "Given a data dump or mbuf buffer, verify the contents of the data\n\n";
    std::cout << "Usage\n";
    std::cout << prog_name << " [options]\n";
    std::cout << "\nWhere options are:\n";
    std::cout << "\t-h,--help\t\tThis help\n";
    std::cout << "\t-v\t\tVerbose output\n";
    std::cout << "\t-m,--mbuf [buffer name]\tRead input from the given mbuf buffer\n";
    std::cout << "\t-s,--mbuf-size [size in MB]\tSize of the mbuf buffer to read\n";
    std::cout << "\t-f,--input-file [file name]\tRead input from a local file\n";
    std::cout << "\t-p\t\tPause before checks\n";
    std::cout << "\t-c [ini file] [par file]\tRead model config from the given ini and par file\n";
    std::cout << "\n";
    std::cout << "You must specify either mbuf or local file input.  If you specify mbuf input\n";
    std::cout << "you must then also specify the size of the mbuf.\n";
    std::cout << "\n";
    std::cout << "You must specify at least one pair of configuration files.  For each pair of\n";
    std::cout << "config files specified all channels listed in the config will be looked for in\n";
    std::cout << "data buffer.  Repeating the -c option is how you specify multiple sets of config\n";
    std::cout << "files.\n";
    std::cout << "\nIt should be noted that this only works with the newer daq_multi_cycle_data_t\n";
...
parse_args(int argc, char* argv[])
{
    ...
    while ( !args.empty() )
    {
        ...
        else if ( arg == "-f" || arg == "--input-file" )
        {
            if ( args.empty() )
            {
                opts.set_error("You must specify a file name to use file input" );
                break;
            }
        ...
        else if (arg == "-c")
        {
            if (args.size() < 2)
            {
                opts.set_error("When specifying a model config to check you must include two files [.ini and .par]");
                break;
            }
...
load_buffer(config_t& opts)
{
    ...
    if (!opts.mbuf_name.empty())
    {
        ...
        volatile void* data = shmem_open_segment(opts.mbuf_name.c_str(), opts.mbuf_size);
        buffer.resize(opts.mbuf_size);

        daq_multi_cycle_header_t* header = (daq_multi_cycle_header_t*)(&buffer[0]);

        std::copy((char *)data, ((char *)data) + opts.mbuf_size,
                buffer.begin());
...
/*!
 * @breif This class looks for a models data in a specifided daq_multi_cycle_data_t
 * buffer.  It ensures that all channels are available in the buffer with their
 * expected values.  It also checks test points, if a test point is marked as active
 * in the data block it will verify the test point data layout and contents for the model.
 */
class CheckDCUData
{
public:
    CheckDCUData(config_t& opts, const daq_multi_cycle_data_t* buffer): opts_(opts), buffer_(buffer)
    {}

    void operator()(const model_channel_list& model)
    {
        if (opts_.verbose)
        {
        ...
        for (int model_index = 0; model_index < slice_header->header.dcuTotalModels; ++model_index)
        {
            if (slice_header->header.dcuheader[model_index].dcuId == model.meta_data.dcuid)
            {
                dcu_header = &(slice_header->header.dcuheader[model_index]);
                break;
            }
            dcu_data_offset += slice_header->header.dcuheader[model_index].dataBlockSize;
            dcu_data_offset += slice_header->header.dcuheader[model_index].tpBlockSize;
        }
        if (!dcu_header)
        {
        ...
        for (int i = 0; i < model.channels.size(); ++i)
        {
            ...
            char* gen_end = model.channels[i]->generate(dcu_header->timeSec,
                dcu_header->timeNSec,
                gen_start);
            ...
            {
                std::cerr << "Data mismatch on " << model.channels[i]->full_channel_name() << "\n";
        ...
        for (int i = 0; i < dcu_header->tpCount; ++i)
        {
            ...
            if (tp_index != 0)
            {
                if (tp_index - 1 >= model.test_points.size())
                {
                    ...
                    throw std::runtime_error("Requesting an invalid test point");
                }
                char* gen_start = &tmp[0];
                char* gen_end = model.test_points[tp_index - 1]->generate(
                    dcu_header->timeSec, dcu_header->timeNSec,
                    gen_start);
                int gen_size = (gen_end - gen_start);
                if ( gen_size != expected_len )
                {
                    std::cerr << "Unexpected TP length of " << gen_size << " bytes\n";
            ...
            if (!std::equal(&tmp[0], (&tmp[0]) + expected_len, data)) {
                ...
                if (tp_index > 0)
                {
                    std::cerr << " " << model.test_points[tp_index-1]->full_channel_name();
                }
                std::cerr << "\n";
...
main(int argc, char* argv[])
{
    ...
    std::transform(opts.configs.begin(), opts.configs.end(),
        std::back_inserter(models), load_channels_for_model);
    ...
    if (opts.verbose)
    {
        std::cout << "Loaded a data buffer with a size of " << data_buffer.size()
                << " bytes\n";
    }

    daq_multi_cycle_data_t* header = (daq_multi_cycle_data_t*)(&data_buffer[0]);

    std::size_t max_size = (header->header.cycleDataSize*header->header.maxCycle)
