...
#define RFMX_MAX_CHANS_PER_RFM	1024
...
volatile TIMING_SIGNAL *pcieTiming;
...
// ************************************************************************
{
  ...
  if(pIpcDataRead[0] != NULL && pIpcDataRead[1] != NULL && pIpcDataRead[2] != NULL && pIpcDataRead[3] != NULL)
  {
	...
	while(!kthread_should_stop()) {
		...
		do{
			msleep(delay);
			newTime = pcieTiming->gps_time;
			counter ++;
		}while(lastTime == newTime && counter < 300);
		lastTime = newTime;
		for (jj=0;jj<RFMX_NUM_DOLPHIN_NETS;jj++) {
			mycounter[jj] = 0;
			for(ii=0;ii<indx;ii++) {
				// Will send port activity info to EPICS via 32 bit ints
				kk = ii/RFMX_CHANS_PER_THREAD;
				mm = ii % RFMX_CHANS_PER_THREAD;
...
inline int copyIpcData (int indx, int netFrom, int netTo, int cycle)
// ************************************************************************
{
  unsigned long syncWord;
  int ii,jj;
...
// ************************************************************************
{
  ...
  if(pIpcDataRead[0] != NULL && pIpcDataWrite[1] != NULL)
  {
	while(!stop_working_threads) {
		...
		do{
			udelay(1);
			newCycle = pcieTiming->cycle;
			counter ++;
		}while(lastCycle == newCycle && counter < 30);
		lastCycle = newCycle;
		if((newCycle + 1) % 4 == 0) {
			copyCycle = (newCycle + 1) % 64;
			udelay(8);
			totalxfers = copyIpcData (0, 0, 1,copyCycle);
			totalxfers = copyIpcData (32, 0, 1,copyCycle);
		}
	}
	printk("%s thread has terminated %d\n","copyRfmDataEX2CS0",1);
	return (void *)0;
...
// ************************************************************************
void *copyRfmDataCS2EX1(void *arg) 
// Thread to copy data chans 31-63 from CS and EX (RFM0)
// ************************************************************************
{
  static int totalxfers;
  unsigned long lastCycle = 0;
  ...
  if(pIpcDataRead[0] != NULL && pIpcDataWrite[1] != NULL)
  {
	while(!stop_working_threads) {
		...
		lastCycle = newCycle;
		if((newCycle + 2) % 4 == 0) {
			copyCycle = (newCycle + 2) % 64;
			totalxfers = copyIpcData (0, 1, 0,copyCycle);
			totalxfers = copyIpcData (32, 1, 0,copyCycle);
		}
	}
	// printk("%s thread has terminated %d\n",((struct params*)data)->name,indx);
	printk("%s thread has terminated %d\n","copyRfmDataCS2EX1",1);
...
// ************************************************************************
void *copyRfmDataCS2EY0(void *arg) 
...
// ************************************************************************
{
  ...
  if(pIpcDataRead[2] != NULL && pIpcDataWrite[3] != NULL)
  {
	while(!stop_working_threads) {
		...
		if((newCycle + 2) % 4 == 0) {
			copyCycle = (newCycle + 2) % 64;
			totalxfers = copyIpcData (0, 2, 3,copyCycle);
			totalxfers = copyIpcData (32, 2, 3,copyCycle);
		}
	}
	printk("%s thread has terminated %d\n","copyRfmDataCS2EY0",1);
	return (void *)0;
  ...
  if(pIpcDataRead[2] != NULL && pIpcDataWrite[3] != NULL)
  {
	while(!stop_working_threads) {
		...
		if((newCycle + 1) % 4 == 0) {
			copyCycle = (newCycle + 1) % 64;
			udelay(8);
			totalxfers = copyIpcData (0, 3, 2,copyCycle);
			totalxfers = copyIpcData (32, 3, 2,copyCycle);
		}
	}
	// printk("%s thread has terminated %d\n",((struct params*)data)->name,indx);
	printk("%s thread has terminated %d\n","copyRfmDataEY2CS1",1);
...
// ************************************************************************
  ...
  pInfo->dolphinCount += 1;
	...
	pcieTiming = (TIMING_SIGNAL *) ((volatile char *) (read_addr) + IPC_PCIE_TIME_OFFSET);
	printk("Time: %ld\tCycle:%ld\n",pcieTiming->gps_time,pcieTiming->cycle);
    }
...
// ************************************************************************
	...
	sprintf(message,"%ld %ld %ld %ld %ld %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
				// EX2CS_CHCNT CS2EX_CHCNT CS2EY_CHCNT EY2CS_CHCNT TIME
						mycounter[0],mycounter[1],mycounter[2],mycounter[3],mycounter[9],
				// EX2CS_ACTIVE1 EX2CS_ACTIVE2 (ACTIVE03)
						myactive[0][0],myactive[0][1],myactive[0][2],
				// CS2EX_ACTIVE1 CS2EX_ACTIVE2 (ACTIVE13)
						myactive[1][0],myactive[1][1],myactive[1][2],
				// CS2EY_ACTIVE1 CS2EY_ACTIVE2 (ACTIVE23)
						myactive[2][0],myactive[2][1],myactive[2][2],
				// EY2CS_ACTIVE1 EY2CS_ACTIVE2 (ACTIVE33)
...
// Setup IPC shared memory with EPICS if needed.
	...
	// Start thread which moves data for RFM0 IPC from EX to CS
	printk("Shutting down CPU 3 at %ld\n",current_time());
	set_fe_code_idle(copyRfmDataEX2CS0,3);
	msleep(100);
	cpu_down(3);

	// Start thread which moves data for RFM0 IPC from CS to EX 
	printk("Shutting down CPU 4 at %ld\n",current_time());
	set_fe_code_idle(copyRfmDataCS2EX1,4);
	msleep(100);
	cpu_down(4);

	// Start thread which moves data for RFM1 IPC from CS to EY
	printk("Shutting down CPU 5 at %ld\n",current_time());
	set_fe_code_idle(copyRfmDataCS2EY0,5);
	msleep(100);
	cpu_down(5);

	// Start thread which moves data for RFM1 IPC from EY to CS
