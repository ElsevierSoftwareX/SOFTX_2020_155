...
#define response_length 4
    ...
    for ( cnt = bread = oread = 0; oread < response_length;
          oread += bread, cnt++ )
    {
        if ( cnt > max_wt )
        {
...
long
read_server_response( int fd )
{
    return read_server_response_wait( fd, 0 );
}
...
listener( void* a )
{
    ...
    */
    ...
    errno = 0;
    if ( bind( listenfd,
               (struct sockaddr*)&daq->listener_addr,
               sizeof( daq->listener_addr ) ) < 0 )
    {
    ...
    for ( ;; )
    {
        ...
        /* Spawn working thread */
        {
            ...
            if ( err_no = pthread_create( &daq->interpreter_tid,
                                          NULL,
                                          (void* (*)(void*))daq->interpreter,
                                          (void*)daq ) )
            {
                strerror_r( err_no, errmsgbuf, sizeof( errmsgbuf ) );
                fprintf( stderr,
                         "pthread_create() failed to spawn interpreter thread; "
                         "err=%s",
                         errmsgbuf );
                close( connfd );
            }
...
daq_initialize( daq_t* daq, int* tcp_port, void* ( *start_func )(void*))
{
    ...
    if ( err_no = pthread_create( &daq->listener_tid,
                                  NULL,
                                  (void* (*)(void*))listener,
                                  (void*)daq ) )
    {
        ...
        strerror_r( err_no, errmsgbuf, sizeof( errmsgbuf ) );
        fprintf( stderr,
                 "pthread_create() failed to spawn a listener thread; err=%s",
                 errmsgbuf );
        pthread_mutex_unlock( &daq->lock );
...
daq_connect( daq_t* daq, char* host, int port )
{
    ...
    if ( !hentp )
    {
        ...
        fprintf( stderr,
                 "Can't find hostname `%s'; gethostbyname(); err=%s\n",
                 host,
                 h_errno );
#endif
        close( daq->sockfd );
    ...
    (void)memcpy( &daq->srvr_addr.sin_addr.s_addr,
                  *hentp->h_addr_list,
                  sizeof( daq->srvr_addr.sin_addr.s_addr ) );
    ...
    if ( !gethostbyname_r( host, &hent, buf, 2048, &gherr ) )
    {
        ...
        fprintf( stderr,
                 "Can't find hostname `%s'; gethostbyname_r(); err=%s\n",
                 host,
                 errmsgbuf );
#endif
        close( daq->sockfd );
    ...
    fflush( stdout );
connect_again:
    if ( connect( daq->sockfd,
                  (struct sockaddr*)&daq->srvr_addr,
                  sizeof( daq->srvr_addr ) ) < 0 )
    {
    ...
    {
        fprintf( stderr,
                 "communication protocol version mismatch: expected %d, "
                 "received %d\n",
                 DAQD_PROTOCOL_VERSION,
                 v );
        close( daq->sockfd );
    ...
    /* Read server response */
    if ( ( rv = read_server_response( daq->sockfd ) ) !=
         DAQD_PROTOCOL_REVISION )
        fprintf( stderr,
                 "Warning: communication protocol revision mismatch: expected "
                 "%d.%d, received %d.%d\n",
                 DAQD_PROTOCOL_VERSION,
                 DAQD_PROTOCOL_REVISION,
                 v,
...
#ifdef VXWORKS
    {
        int blen = 4 * 4096;
        if ( setsockopt(
                 daq->sockfd, SOL_SOCKET, SO_SNDBUF, &blen, sizeof( blen ) ) <
             0 )
        {
...
/*
  ...
  Returns -2 on channel reconfiguration. Special reconfiguration data block was
  received from the server. For the client it meens he need to reread channel
  data conversion variables and status from the *daq structure.
...
daq_recv_block( daq_t* daq )
{
    ...
    if ( seconds == 0xffffffff )
    {
        ...
        for ( i = 0; i < nchannels; i++ )
        {
            if ( read_bytes( daq->datafd,
                             (char*)&daq->s[ i ].signal_offset,
                             sizeof( float ) ) != sizeof( float ) )
                return DAQD_ERROR;
            *( (unsigned long*)&daq->s[ i ].signal_offset ) =
                ntohl( *( (unsigned long*)&daq->s[ i ].signal_offset ) );
            if ( read_bytes( daq->datafd,
                             (char*)&daq->s[ i ].signal_slope,
                             sizeof( float ) ) != sizeof( float ) )
                return DAQD_ERROR;
            *( (unsigned long*)&daq->s[ i ].signal_slope ) =
                ntohl( *( (unsigned long*)&daq->s[ i ].signal_slope ) );
    ...
    /* block length does not include the length of itself while the header
     * length does */
    block_len -= HEADER_LEN;
        ...
        *num_channels_received = channels;
        ...
        for ( i = 0; i < channels; i++ )
        {
            ...
            if ( s )
                *s = 0;
            strncpy(
                channel[ i ].s.signal_units, buf, MAX_ENGR_UNIT_LENGTH - 1 );
...
#undef READ_STRING
    ...
    *num_channels_received = channels;
    ...
    for ( i = 0; i < channels; i++ )
    {
        ...
        if ( read_bytes( daq->sockfd,
                         channel[ i ].name,
                         MAX_CHANNEL_NAME_LENGTH ) != MAX_CHANNEL_NAME_LENGTH )
            return DAQD_ERROR;

        /* null terminate the name; strip all trailing white space */
        for ( cptr = channel[ i ].name + MAX_CHANNEL_NAME_LENGTH - 1;
              isspace( *cptr ) && cptr > channel[ i ].name;
              cptr-- )
            ;
        *++cptr = '\000';
        ...
        if ( ( channel[ i ].group_num = read_server_response( daq->sockfd ) ) <
             0 )
            return DAQD_ERROR;

        if ( ( channel[ i ].data_type = read_server_response( daq->sockfd ) ) <
             0 )
            return DAQD_ERROR;
        ...
        if ( ( *( (unsigned long*)&channel[ i ].s.signal_gain ) =
                   daq_recv_id( daq ) ) < 0 )
            return DAQD_ERROR;

        if ( ( *( (unsigned long*)&channel[ i ].s.signal_slope ) =
                   daq_recv_id( daq ) ) < 0 )
            return DAQD_ERROR;

        if ( ( *( (unsigned long*)&channel[ i ].s.signal_offset ) =
                   daq_recv_id( daq ) ) < 0 )
            return DAQD_ERROR;

        if ( read_bytes( daq->sockfd,
                         channel[ i ].s.signal_units,
                         MAX_ENGR_UNIT_LENGTH ) != MAX_ENGR_UNIT_LENGTH )
            return DAQD_ERROR;

        /* null terminate units string; strip all trailing white space */
        for ( cptr = channel[ i ].s.signal_units + MAX_ENGR_UNIT_LENGTH - 1;
              isspace( *cptr ) && cptr > channel[ i ].s.signal_units;
              cptr-- )
            ;
        *++cptr = '\000';
    }
...
/*
  ...
  Returns zero on success.
  Sets `*num_channel_groups_received' to the actual number of configured channel
  groups.
*/
int
daq_recv_channel_groups( daq_t*               daq,
    ...
    *num_channel_groups_received = groups;
    ...
    for ( i = 0; i < groups; i++ )
    {
        ...
        for ( cptr = group[ i ].name + MAX_CHANNEL_NAME_LENGTH - 1;
            ...
            ;
        *++cptr = '\000';
        if ( ( group[ i ].group_num = read_server_response( daq->sockfd ) ) <
             0 )
            return DAQD_ERROR;
    }
    return 0;
...
/*
  Receive some ID, coded as two server responses, ie two groups, four hex digits
  in each
*/
unsigned long
daq_recv_id( daq_t* daq )
{
    return read_server_response( daq->sockfd )
        << ( sizeof( unsigned long ) * CHAR_BIT / 2 ) |
        read_server_response( daq->sockfd );
}
