...
// as of ldas_tools 2.5, the LDAS_VERSION is no longer provided. Instead use
// FRAMECPP_VERSION_NUMBER
#if USE_LDAS_VERSION
...
/// Daqd server main class. This is a top-level class, which encloses various
/// objects representing threads of execution.
class daqd_c
{
public:
    enum
    {
        ...
        max_listeners = 3,
        max_password_len = 8,
        frame_buf_size = 10 * 1024 *
            1024 // Should it be calculated based on saved data size?
    };
...
public:
    daqd_c( )
        : _framer_work_queue( 0 ), _science_framer_work_queue( 0 ),
          ...
          num_gds_channels( 0 ), num_gds_channel_aliases( 0 ),

          frame_saver_tid( 0 ), science_frame_saver_tid( 0 ),
          frame_saver_io_tid( 0 ), science_frame_saver_io_tid( 0 ),
          data_feeds( 1 ), dcu_status_check( 0 ),

          writer_sleep_usec( 1000000 ), main_buffer_size( 16 ),
          ...
          detector_arm_x_midpoint( .0 ), detector_arm_y_midpoint( .0 ),

          detector_name1( (char*)"" ), detector_prefix1( (char*)"" ),
          detector_longitude1( 0. ), detector_latitude1( 0. ),
          detector_elevation1( .0 ), detector_arm_x_azimuth1( .0 ),
          detector_arm_y_azimuth1( .0 ), detector_arm_x_altitude1( .0 ),
          detector_arm_y_altitude1( .0 ), detector_arm_x_midpoint1( .0 ),
...

    {
        ...
        { // Ifo number
            for ( int j = 0; j < DCU_COUNT; j++ )
            {
                ...
                dcuRate[ i ][ j ] =
                    ( IS_2K_DCU( j ) || j == DCU_ID_EX_2K || j == DCU_ID_TP_2K )
                    ? 2048
                    : 16384;
    ...
    void queue_frame_checksum( const char*                     frame_filename,
    ...
    // NOTE: move_addr can be used to capture the start of the DCU DQ data area.
    // But due to the many ways to build daqd this is only guaranteeded to work
    // on the DAQD_SHMEM build of daqd.
    void initialize_vmpic( unsigned char**   _move_buf,
                           int*              _vmic_pv_len,
                           struct put_dpvec* vmic_pv,
                           dcu_move_address* move_addr = 0 );
    ...
    /// time to filename map and place to keep timestamps describing the data we
    /// have
    filesys_c fsd;
    /// science frames time to filename map and place to keep timestamps
    ...
    char password[ max_password_len +
                   1 ]; ///< Most of the commands are password protected

    char* config_file_name; ///< Name of the startup config file (~/.daqdrc is
                            ///< used if not set)
    ...
    unsigned int
        dcu_status_check; /* 1 - check ifo 0; 2- ifo 1; 3 - both; 0 - none */
    ...
    static void set_thread_priority( char* thread_name,
                                     char* thread_abbrev,
                                     int   rt_priority,
                                     int   cpu_affinity );
    ...
    /// Indicates whether the DCU is configured or not; holds DCU data size. One
    /// for each ifo.
    ...
    /// DCU Inter-processor communication area pointer into an RFM board; for
    /// each ifo
    volatile rmIpcStr* dcuIpc[ 2 ][ DCU_COUNT ];

    static unsigned long fr_cksum( string         cksum_file,
                                   char*          tmpf,
                                   unsigned char* data,
                                   int            image_size );
    ...
    int update_archive( char*         name,
                        unsigned long gps,
                        unsigned long dt,
                        unsigned int  dir_num );
    int
    configure_archive_channels( char* archive_name, char* file_name, char* );
