...
public:
    ...
    thread_stacksize_t& operator=(const thread_stacksize_t&) throw() = default;
    thread_stacksize_t& operator=(const std::size_t new_size) throw()
    {
        size_ = new_size;
        return *this;
    }
...
/*!
 * @brief a RAII wrapper for pthread_attr_t, ensuring that it is always destroyed.
 */
class thread_attr_t {
public:
    thread_attr_t(): attr_{}
    ...
    void set(thread_scope_t scope)
    {
        pthread_attr_setscope(&attr_, static_cast<int>(scope));
    }
    void set(thread_stacksize_t stack_size)
    {
 ...
 * @details The handler is given a stop function which is called (once) to stop all the
 * threads that are associated with it.  It is the responsibility of the caller to ensure
 * that the stop function will indeed stop all the threads that will be managed by
 * the thread_handler_t.
 */
...
public:
    ...
    explicit thread_handler_t(stop_funtion_t& stopper): stopper_{std::move(stopper)} {};
    ...
    void push_back(thread_action_t action, thread_attr_t& attr)
    {
        std::lock_guard<std::mutex> l_{m_};
        std::vector<pthread_t> ids(thread_ids_.begin(), thread_ids_.end());
        ids.emplace_back();
        if (launch_pthread(ids.back(), attr.at(), std::move(action)) != 0)
        {
