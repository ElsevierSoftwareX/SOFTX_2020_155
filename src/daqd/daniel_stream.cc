    ...
    for ( ;; )
    {
        ...
        */
        ...
        for ( ;; )
        {
            ...
            // printf("offs=%d\n", ((char *)&mxDataBlock) - (char
            // *)&(mxDataBlock.mxIpcData.dcuId));
...
struct daqMXdata {
	...
	char mxDataBlock[DAQ_DCU_BLOCK_SIZE];
		shmDataPtr = (char *)(dcu_addr + CDS_DAQ_NET_DATA_OFFSET);
            ...
            // Send
            // if (sendto(s, &mxDataBlock, sendLength, 0, &si_other, slen)==-1)
            // diep("sendto()");
            radio_buffer* buf = 0;
            ...
            {
                fprintf( stderr,
                         "cannot find unused radio buffer; data was lost" );
                exit( 1 );
            }
            ...
            radio.send(
                buf->data, sendLength, &buf->inUse, false, lastCycle, 1 );
            //	printf("%d\n", lastCycle);
        }
    }
}
...
main( int argc, char** argv )
{
    ...
    while ( ( c = getopt( argc, argv, "hd:s:" ) ) != EOF )
        switch ( c )
        {
        case 'd':
            rem_host = optarg;
    ...
    // Map shared memory
    dcu_addr = (unsigned char*)mmap(
        0, 64 * 1024 * 1024 - 5000, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0 );
    if ( dcu_addr <= 0 )
    {
    ...
    shmDataPtr = (char*)( dcu_addr + CDS_DAQ_NET_DATA_OFFSET );
    shmTpTable = (struct cdsDaqNetGdsTpNum*)( dcu_addr +
    ...
    // Open multicast port
    if ( !radio.open(
             mcast_addr, mcast_interface, mcast_port + shmIpcPtr->dcuId ) )
    {
        fprintf( stderr, "framexmit open failed" );
        exit( 1 );
    }

    std::cerr << "opened framexmit addr=" << mcast_addr
              << " iface=" << ( mcast_interface ? mcast_interface : "" )
              << std::endl;
