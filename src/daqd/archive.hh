...
private:
  ...
  struct channel *add_channel() {
    if (nchannels%channel_alloc_chunk == 0) {
      channels = (struct channel *) realloc(channels, (nchannels + channel_alloc_chunk) * sizeof(struct channel));
    }
  ...
  class locker {
    archive_c *dp;
  public:
    locker (archive_c *objp) {(dp = objp) -> lock ();}
    ~locker () {dp -> unlock ();}
  };
};
