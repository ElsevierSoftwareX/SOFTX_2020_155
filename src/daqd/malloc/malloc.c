...
pthread_mutex_t dlMALLOc_lock = PTHREAD_MUTEX_INITIALIZER;

//   pthread_mutex_lock (&dlMALLOc_lock);
...
#endif
{
  Void_t* ptr;
  pthread_mutex_lock (&dlMALLOc_lock);
  ptr = unsafe_mALLOc(bytes);
  pthread_mutex_unlock (&dlMALLOc_lock);
  return ptr;
}

#if __STD_C
Void_t* unsafe_mALLOc(size_t bytes)
#else
Void_t* unsafe_mALLOc(bytes) size_t bytes;
#endif
{
...
#if __STD_C
void unsafe_fREe(Void_t* mem)
#else
void unsafe_fREe(mem) Void_t* mem;
#endif
{
...
Void_t* unsafe_rEALLOc(Void_t* , size_t );
...
#endif
{
  Void_t* ptr;
  pthread_mutex_lock (&dlMALLOc_lock);
  ptr = unsafe_rEALLOc(oldmem, bytes);
  pthread_mutex_unlock (&dlMALLOc_lock);
  return ptr;
}

#if __STD_C
Void_t* unsafe_rEALLOc(Void_t* oldmem, size_t bytes)
#else
Void_t* unsafe_rEALLOc(oldmem, bytes) Void_t* oldmem; size_t bytes;
#endif
{
  ...
  if (bytes == 0) { unsafe_fREe(oldmem); return 0; }
#endif

  if ((long)bytes < 0) return 0;

  /* realloc of null is supposed to be same as malloc */
  if (oldmem == 0) return unsafe_mALLOc(bytes);
 ...
 split:  /* split off extra room in old or expanded chunk */
  ...
  if (newsize - nb >= MINSIZE) /* split off remainder */
  {
    ...
    unsafe_fREe(chunk2mem(remainder)); /* let free() deal with it */
  }
  else
  {
...
#endif
{
  ...
  if (alignment <= MALLOC_ALIGNMENT) return unsafe_mALLOc(bytes);
  ...
  nb = request2size(bytes);
  m  = (char*)(unsafe_mALLOc(nb + alignment + MINSIZE));
  ...
  if (remainder_size >= (long)MINSIZE)
  {
    ...
    set_head(remainder, remainder_size | PREV_INUSE);
    set_head_size(p, nb);
    unsafe_fREe(chunk2mem(remainder));
  }
