...
crc32_calc(const void* buf, uint32_t len, uint32_t crc) {
    ...
    while (len >= 8) {
        ...
        crc = crctab_8byte[7][ one & 0xFF] ^
              ...
              crctab_8byte[4][ one>>24 ] ^
              crctab_8byte[3][ two & 0xFF] ^
...
//======================================  Constructor
checksum_crc32::checksum_crc32(void) {
    crc32_init();
    reset();
}
...
//======================================  Add data to checksum
void
checksum_crc32::add(const void* buf, uint32_t length) {
    _value = crc32_calc(buf, length, _value);
    _length += length;
}

//======================================  Reset calculation
void
checksum_crc32::reset(void) {
    _value  = 0;
    _length = 0;
}
...
uint32_t
checksum_crc32::result(void) const {
    return crc32_finish(_length, _value);
}
