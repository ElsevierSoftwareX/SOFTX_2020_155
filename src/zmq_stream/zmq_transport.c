 ...
 * @return 0 on error
 *
 * @note The 'compressed' stream just skips unneeded TP table entries and DCU entries.
 */
int zmq_send_daq_multi_dcu_t(daq_multi_dcu_data_t *src, void *socket, int *xmit_size)
{
    int cur_model = 0;
    int total_models = 0;
    char *dest_data = 0;
    ...
    if (!src || !socket) return 0;
    total_models = _dest_buffer.header.dcuTotalModels = src->header.dcuTotalModels;
    ...
    for (cur_model = 0; cur_model < total_models; ++cur_model)
    {
        ...
        /* copy over the header, including only the portion of the TP table that is used */
        header_size = sizeof(daq_msg_header_abrev_t)+sizeof(unsigned int)*(src->header.dcuheader[cur_model].tpCount);
    ...
    transfer_size = (char*)dest_data - (char*)(&_dest_buffer) + src->header.fullDataBlockSize;
 ...
 * @note The 'compressed' stream just skips unneeded TP table entries and DCU entries.
 */
int zmq_recv_daq_multi_dcu_t(daq_multi_dcu_data_t *dest, void *socket)
{
    zmq_msg_t message;
    int size = 0;
    int cur_model = 0;
