...
int
dc_generate_connection_string( char*       dest,
                               const char* src,
                               size_t      dest_len,
...
int
dc_set_zmq_options( void* z_socket )
{
    char* env = 0;
    int   i_value = 0;
    ...
    if ( env )
    {
        i_value = atoi( env );
        zmq_setsockopt(
            z_socket, ZMQ_MULTICAST_MAXTPDU, &i_value, sizeof( int ) );
    }
    env = getenv( "ZMQ_RATE" );
    if ( env )
    {
    ...
    if ( env )
    {
        i_value = atoi( env );
        zmq_setsockopt(
            z_socket, ZMQ_CONNECT_TIMEOUT, &i_value, sizeof( int ) );
    }
    return 1;
}
