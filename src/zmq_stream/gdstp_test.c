...
unsigned int wait_delay = 1; // Wait before acknowledging sends with mx_wait()
...
main( int argc, char** argv )
{
    ...
    while ( ( c = getopt( argc, argv, "hd:e:s:Vvw:x" ) ) != EOF )
        switch ( c )
        {
        case 's':
            sysname = optarg;
    ...
    for ( unsigned int i = 0; i < nsys; i++ )
    {
        ...
        gdsStatus[ i ] =
            (GDS_STATUS*)( dcu_addr + GDS_DATA_OFFSET - sizeof( GDS_STATUS ) );
    ...
    do
    {
        ...
        {
            gdsHdrBlockPtr[ ii ] =
                (char*)( gdsDataAreaPtr[ ii ] + ( GDS_BUFF_SIZE * lastCycle ) );
            ;
            gdsHdrPtr[ ii ] = (GDS_DATA_HEADER*)( gdsHdrBlockPtr[ ii ] );
            gdsDataBlockPtr[ ii ] =
                (char*)( gdsHdrBlockPtr[ ii ] + sizeof( GDS_DATA_HEADER ) );
            if ( gdsHdrPtr[ ii ]->chan_count > 0 )
            {
                printf( "%s - New cycle found %d with %d GDS chans\n",
                        modelnames[ ii ],
                        lastCycle,
                        gdsHdrPtr[ ii ]->chan_count );
                for ( jj = 0; jj < gdsHdrPtr[ ii ]->chan_count; jj++ )
                {
                    gdsdata[ ii ] = (float*)gdsDataBlockPtr[ ii ] +
                        ( jj * gdsValsPerChan[ ii ] );
                    sprintf(
                        channel_name, "%s", gdsHdrPtr[ ii ]->chan_name[ jj ] );
