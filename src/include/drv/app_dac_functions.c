...
inline int app_dac_write(int ioMemCtrDac, int ioClkDac, dacInfo_t *dacinfo) {
  ...
  for (jj = 0; jj < cdsPciModules.dacCount; jj++) {
    ...
    /// - -- If user app < 64k rate (typical), need to upsample from code rate
    /// to IOP rate
    for (kk = 0; kk < OVERSAMPLE_TIMES; kk++) {
      ...
      for (ii = 0; ii < num_outs; ii++) {
        ...
        dacOut[jj][ii] *= -1;
#endif
        /// - ---- Read in DAC value, if channel is used by the application, and
        /// run thru upsample filter\n
        if (dacOutUsed[jj][ii]) {
#ifdef OVERSAMPLE_DAC
#ifdef NO_ZERO_PAD
          /// - --------- If set to NO_ZERO_PAD (not standard setting), then DAC
          /// output value is repeatedly run thru the filter OVERSAMPE_TIMES.\n
          dac_in = dacOut[jj][ii];
          dac_in =
              iir_filter_biquad(dac_in, FE_OVERSAMPLE_COEFF, 2,
        ...
        if (!iopDacEnable)
          dac_out = 0;
        /// - ---- Load last values to EPICS channels for monitoring on GDS_TP
        /// screen.
        ...
        /// - --------- Only write to DAC channels being used to allow two or
        /// more slaves to write to same DAC module.
        if (dacOutUsed[jj][ii])
