...
/************************************************************************/
inline void updateEpics(int subcycle, FILT_MOD *dsp, FILT_MOD *pDsp,
                        COEF *dspCoeff, VME_COEF *pCoeff) {
  int ii;
...
inline void updateFmSetpoints(FILT_MOD *dsp, FILT_MOD *pDsp, COEF *dspCoeff,
                              VME_COEF *pCoeff) {
  int ii;
  ...
  for (ii = 0; ii < MAX_MODULES; ii++) {
    ...
    if (dsp->inputs[ii].mask &
        0x80000000) { /* Ramp time controlled by the FE */
      pDsp->inputs[ii].gain_ramp_time = dsp->inputs[ii].gain_ramp_time;
    } else {
      dsp->inputs[ii].gain_ramp_time = pDsp->inputs[ii].gain_ramp_time;
    }
