...
inline int app_adc_read(int ioMemCtr, int ioClk, adcInfo_t *adcinfo,
                        int cpuClk[]) {
  int ii, jj;
  ...
  for (jj = 0; jj < cdsPciModules.adcCount; jj++) {
    ...
    /// - --------- If data not ready in time, set error, release DAC channel
    /// reservation and exit the code.
    ...
    for (ii = 0; ii < MAX_ADC_CHN_PER_MOD; ii++) {
      ...
      if (dWordUsed[jj][ii]) {
        dWord[jj][ii] = iir_filter_biquad(dWord[jj][ii], FE_OVERSAMPLE_COEFF, 2,
                                          &dHistory[ii + jj * 32][0]);
      }
