...
inline int iop_adc_init(adcInfo_t *adcinfo) {
  ...
  for (jj = 0; jj < cdsPciModules.adcCount; jj++) {
    ...
    // Preload input memory with dummy variables to test that new ADC data has
    // arrived.
    adcDummyData = (int *)cdsPciModules.pci_adc[jj];
    // Write a dummy 0 to first ADC channel location
    // This location should never be zero when the ADC writes data as it should
    // always have an upper bit set indicating channel 0.
    *adcDummyData = 0x0;
    if (cdsPciModules.adcType[jj] == GSC_18AISS6C)
      adcDummyData += 5;
    else
      adcDummyData += 31;
    // Write a number into the last channel which the ADC should never write ie
    // no upper bits should be set in channel 31.
...
inline int iop_adc_read(adcInfo_t *adcinfo, int cpuClk[]) {
  ...
  // Read ADC data
  for (jj = 0; jj < cdsPciModules.adcCount; jj++) {
    /// - ---- ADC DATA RDY is detected when last channel in memory no longer
    /// contains the dummy variable written during initialization and reset
    /// after the read.
    ...
    do {
      /// - ---- Need to delay if not ready as constant banging of the input
      /// register will slow down the ADC DMA.
      cpuClk[CPU_TIME_ADC_WAIT] = rdtsc_ordered();
      adcinfo->adcWait =
          (cpuClk[CPU_TIME_ADC_WAIT] - cpuClk[CPU_TIME_RDY_ADC]) / CPURATE;
      /// - ---- Allow 1sec for data to be ready (should never take that long).
    } while ((*packedData == DUMMY_ADC_VAL) &&
             (adcinfo->adcWait < MAX_ADC_WAIT));

    /// - ---- Added ADC timing diagnostics to verify timing consistent and all
    /// rdy together.
    if (jj == 0) {
      adcinfo->adcRdTime[jj] =
          (cpuClk[CPU_TIME_ADC_WAIT] - cpuClk[CPU_TIME_CYCLE_START]) / CPURATE;
      if (adcinfo->adcRdTime[jj] > 1000)
        adcStat = ADC_BUS_DELAY;
      if (adcinfo->adcRdTime[jj] < 13)
    ...
    if (jj == 0) {
      ...
      /// \> If first cycle of a new second, capture IRIG-B time. Standard for
      /// aLIGO is TSYNC_RCVR.
      if (cycleNum == 0) {
        ...
        if (cdsPciModules.gpsType == TSYNC_RCVR) {
          ...
          /// nanoseconds to be read later (on next cycle). Two step process
          /// used to save CPU time here, as each read can take 2usec or more.
