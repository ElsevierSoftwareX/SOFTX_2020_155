...
#define MAX_ADC_WAIT                                                           \
  1000000 // Max time (usec) to wait for ADC data transfer in iop app
#define MAX_ADC_WAIT_CARD_0                                                    \
  23 // Max time (usec) to wait for 1st ADC card data ready
#define MAX_ADC_WAIT_C0_32K                                                    \
  36 // Max time (usec) to wait for 1st ADC card data ready on 32K IOP
#define MAX_ADC_WAIT_CARD_S                                                    \
  5 // Max time (usec) to wait for remaining ADC card data ready
#define MAX_ADC_WAIT_ERR_SEC                                                   \
  3 // Max number of times ADC time > WAIT per sec before alarm set.
#define MAX_ADC_WAIT_SLAVE                                                     \
  100000 // Max time (usec) to wait for ADC data transfer in slave app
#define MAX_ADC_WAIT_CYCLE 17
#define DUMMY_ADC_VAL                                                          \
  0xf000000 // Dummy value for test last ADC channel has arrived
#define ADC_1ST_CHAN_MARKER                                                    \
  0xf0000 // Only first ADC channel should have upper bits set as first chan
          // marker.
...
// Timing control register definitions for use with Contec1616 control of timing
// slave.
...
typedef struct PLX_9056_DMA {
  unsigned int
      pad[32]; /* DMA register is at 0x80 offset from base of PLX chip */
...
typedef struct CDS_HARDWARE {
  ...
  int cDio6464lCount; /* Number of Contec isolated 32 channel DIO modules found
                       */
  unsigned short
      pci_cdo_dio1[MAX_DIO_MODULES]; /* io regs of Contec 32BO mods */
