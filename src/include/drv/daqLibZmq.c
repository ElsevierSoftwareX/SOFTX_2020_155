...
#undef iir_filter
    ...
    /// ie must be loaded and available to the realtime application when the
    /// next code cycle is initiated.
    ...
    /* Done with 1/16 second DAQ data block */
    {
      ...
      if (daqBlockNum == 15) {
        ...
        memcpy(excnum, (const void *)(gdsPtr->tp[_2k_sys_offs][0]),
               sizeof(excnum));
        memcpy(tpnum, (const void *)(gdsPtr->tp[2 + _2k_sys_offs][0]),
               sizeof(tpnum));
...
int daqConfig(DAQ_INFO_BLOCK *dataInfo, DAQ_INFO_BLOCK *pInfo, char *pEpics) {
  ...
  printf("DAQ EPICS: Int = %d  Flt = %d Filters = %d Total = %d Fast = %d\n",
         dataInfo->numEpicsInts, dataInfo->numEpicsFloats,
         dataInfo->numEpicsFilts, dataInfo->numEpicsTotal, dataInfo->numChans);
